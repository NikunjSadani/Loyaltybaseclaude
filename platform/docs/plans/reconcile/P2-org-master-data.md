...
| RF5 | **Med** | `prisma/schema.prisma:899` | `Outlet.outletCode @unique` is **globally** unique → conflicts with the P2 modeling note that one physical outlet may exist as separate per-tenant records. Should likely be `@@unique([clientId, outletCode])` (via partner→client). | ✅ FIXED in 2.4 — `Outlet` now has its own `clientId` + `@@unique([clientId, outletCode])`; all outlet lookups updated to the compound key (also tightened isolation in `kyc/not-interested`, `visibility/bulk-upload`). |
| RF7 | **Med** | `prisma/schema.prisma` `SalesUser.employeeCode @unique` | Globally unique employeeCode → if two tenants upload the same Employee ID, the 2.1 hierarchy upsert (keys on `employeeCode`) would match + **overwrite the other tenant's SalesUser**. **Surfaced by the 2.1 audit.** | ✅ FIXED in 2.4 — `SalesUser` now has `clientId` + `@@unique([clientId, employeeCode])`; `persistHierarchy` upsert key updated. |
