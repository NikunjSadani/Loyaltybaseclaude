...
| RF7 | **Med** | `prisma/schema.prisma` `SalesUser.employeeCode @unique` | Globally unique employeeCode → if two tenants upload the same Employee ID, the 2.1 hierarchy upsert (keys on `employeeCode`) would match + **overwrite the other tenant's SalesUser**. Latent (single-tenant demo safe). Fix needs a `clientId` column on `SalesUser` + `@@unique([clientId, employeeCode])`, and the persist upsert key updated. **Surfaced by the 2.1 audit.** Bundle with the outlet-wave migration (alongside nullable `partnerId` + RF5). |
