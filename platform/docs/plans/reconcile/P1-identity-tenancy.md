...
| 1.4a (secret-resolution fix) | ✅ committed | tsc 0 / lint clean / full suite no new reds (vitest `server-only` alias safe) | **PASS — F8 CLOSED** (DB path == registry secret both env-set/unset; resolveMsg91AuthKey gone; no cycles). Notes: live-env-set case untested (module-load env eval); `vitest.integration.config.ts` lacks the `server-only` alias (latent — no live test imports the module yet) |
