...
| **Pagination Wave 2 — remaining lists** | Wave 1 done (`2d1a50e`: `/admin/outlets`, `/admin/credits/batches` + `/reversals`). Recommendation for W2 = **invoices + schemes-FE-adoption only**; **skip** paginating the tiny KPI / banner / partner-sales user-scoped lists (low value — add a `@Max` cap instead). **PENDING owner decision on scope.** | When any tenant's lists grow large | 2026-07-01; #26; P8.3 |
