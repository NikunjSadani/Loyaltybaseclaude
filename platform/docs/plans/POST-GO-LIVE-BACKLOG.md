...
| **WhatsApp KYC generalization** — the KYC submit/approve WhatsApp notifications are currently **per-tenant config-gated to deoleo** via `whatsapp-kyc.config.ts` (hardcoded template names + integrated number). Generalize to a **per-tenant settings UI** (template names, recipient rules, on/off). | Built deoleo-only by config (WHATSAPP-KYC, `3900af3`); no other tenant needs it at launch. | When a **2nd client** needs WhatsApp KYC notifications | small–medium | 2026-06-30; `3900af3` |
...
| **Admin `/admin/users` hardening** (from the audit, non-blocking) — list **pagination** (backend already paginates), a **self-deactivate guard**, and sourcing the **"creating in `<tenant>`" label from server-truth** instead of localStorage; align the old `gifsy/users` hardcoded-Active column. | Build shipped FE-only (ADMIN-UI, `d306129`); these are polish items the audit flagged as non-blocking. | Fast-follow / when admin-user volume or tenant-label correctness matters | small | 2026-06-30; `d306129` |
