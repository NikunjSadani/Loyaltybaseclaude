...
**✅ LIVE IN PROD — cutover #15 (`bda9bf3`, 2026-07-27).** (1) **✅ DONE (2026-07-26)** — guarded staging pre-check + cleanup: `scheme_enrollments` held **1** `w3test-cpB` residue row (2026-07-23, login-less sibling, status ACTIVE; backup-logged in the `gifsy-oneoff-staging` job logs) → deleted (guarded: `current_database()='gifsy_staging'` + row-count cap) → **now 0, staging is push-ready** (the migration's abort-guard will pass); (2)(3) **✅ DONE 2026-07-26** — pushed `develop` (`29e44f0`); staging deployed (both services on `staging-29e44f0`, `/health/ready` db:up) + migration verified applied (`scheme_outlets`/`scheme_submissions`/`scheme_enrollment_form_versions`/`scheme_broadcasts` present; `scheme_enrollments.partnerId` dropped + `schemeOutletId` added; `OtpPurpose.SCHEME_ENROLL_CONSENT` present). (4) **✅ STAGING RUNTIME-VERIFIED 2026-07-26** (real cross-role logins, FIXED_OTP): Gifsy assume-Deoleo → create ACTIVE scheme → versioned enrollment-form v1 → outlet self-enroll (lazy-create → SUBMITTED) → **sales slice** (`sales/eligible`=3, `sales-targets`=7 — gap fix confirmed) → admin reject → partner resubmit (**v2**) → admin sees the captured note cross-session. (5) **✅ PROD CUTOVER DONE (`bda9bf3`, 2026-07-27):** guarded prod pre-check (`scheme_enrollments`=0, `scheme_outlets` absent, partnerId col+unique index present → abort-guard clears) → FF-merge `develop`→`main` → owner-approved the prod gate → CI `gifsy-migrate` applied `20260725120000_scheme_data_collection` (`--wait`) → **PROD VERIFIED:** both services serve `bda9bf3` @100% traffic (api `00032-sgx`, fe `00024-vgp`), `/health/ready` `{db:up}`, migration `done:true` + all 4 new tables + `partnerId` dropped + `schemeOutletId`/`submittedByUserId` added + `SchemeEnrollmentStatus` enum + `OtpPurpose.SCHEME_ENROLL_CONSENT` + `schemes.audienceConfig` + `scheme_enrollment_forms.version` all present, `KYC_CLEANUP_SECRET` bound (cutover-#14 self-heal watch CLEARED), `deoleoloyalty.gifsy.in/auth/login` 200, `GET /v1/schemes` 401 (wired, not 404). **Additive + dormant** — no prod schemes exist until a Gifsy admin creates one. **REMAINING = owner UAT + the ~10-min real-phone smoke** (device camera/geo + phone-OTP-owner-pin — the one path not verifiable without a handset). **✅ CI-hygiene fix DONE (`e46dd51`, 2026-07-27):** `ci.yml`'s api-job `tsc --noEmit` step was red on a pre-existing tsconfig `rootDir`/`include` mismatch (bare `tsc` → `tsconfig.json` rootDir=src + default include=`**/*` → TS6059 on out-of-src `prisma.config.ts`/`prisma/*.ts`/`test/*.ts`; unrelated to scheme code, never gated a deploy — deploy workflows run `npm test` only, prod builds via `nest build`). Repointed to `tsc --noEmit -p tsconfig.build.json` (deployable set); verified exit 0 + full gate green (jest 1826 · nest 0 · vitest 1941 · platform tsc 0).
