...
- [ ] **Custom domain + tenant resolution (Deoleo = `deoleoloyalty.gifsy.in`):** DNS record → Cloud Run/load-balancer (terraform `load-balancer.tf`/`cloud-run.tf`) + Google-managed SSL. ⚠️ **BLOCKER caught 2026-06-20:** `auth/login/actions.ts:resolveClientId()` uses the subdomain's **first label AS the clientId** (`deoleo.gifsy.in`→`deoleo`). The branded domain `deoleoloyalty.gifsy.in` → `deoleoloyalty` ≠ the DB clientId `deoleo` → **login fails for everyone on that domain.** Resolve BEFORE pointing the domain: either (a) use `deoleo.gifsy.in`, or (b) add a domain→clientId map (ideally a `domain`/`hostnames` column on the `clients` table — config not code), or (c) rename the tenant clientId (risky, all data keyed on it). Recommend (b). (Owner asked 2026-06-20.)
