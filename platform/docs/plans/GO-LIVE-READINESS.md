...
- [ ] **Custom domain (Deoleo = `deoleoloyalty.gifsy.in`):** DNS record → Cloud Run/load-balancer (terraform `load-balancer.tf`/`cloud-run.tf`) + Google-managed SSL. ✅ **Tenant-resolution handled (`5de8aa9`, 2026-06-20):** the branded-domain→clientId mismatch is fixed — `CLIENT_REGISTRY` carries `domains: ['deoleoloyalty.gifsy.in']` and `resolveSlugFromHostname`/login map it → `deoleo` (a future tenant's domain = a one-line registry edit; DB-column version → BACKLOG §A). **Remaining = infra only:** (1) DNS + LB + managed SSL for the domain; (2) ⚠️ confirm the load balancer **preserves the original `Host` header** to Cloud Run (the resolver reads it server-side — if the LB rewrites Host, tenant resolution breaks); (3) smoke-test login on the real domain.
