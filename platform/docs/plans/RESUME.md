...
**🚀 VISIBILITY-LED PAYOUTS: 194C AUTO-INVOICING + CONFIGURABLE TDS — ✅ LIVE ON STAGING (develop `500eaf9`) + runtime-verified; ▶ NEXT = owner UAT → W4 owner-gated prod cutover.** Both migrations applied to gifsy_staging (verified); API-surface config-round-trip + reports + RBAC verified live; Deoleo staging tdsPolicy=194C GROSS_UP (armed, no-op until a VISIBILITY payout flows). REMAINING = owner UAT (real payout-Excel write-path E2E) → prod cutover.
