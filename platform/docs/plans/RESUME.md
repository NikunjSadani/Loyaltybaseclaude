...
## ▶▶ START HERE (post-compact) — CLEAN STATE: cutover #22 is LIVE; develop == main; no pending build
**prod = main = origin/develop = `fb996d8` (cutover #22 LIVE, 2026-07-31).** Verify HEADs via `git log` — there is NO batch waiting on staging; develop is NOT ahead of prod. Everything from cutover #22 (the 5-part payload — reactivate pending-outlet leak fix, enrollment Excel report rebuild with the public tokenized `/v1/schemes/media/view` media links, the PARKED remove-from-KYC-queue outlet state with the applied-to-prod `outlet_kyc_intent_parked` enum migration, grouped-child GST-cert/cheque doc carry-forward, "Activations / Tasks" label rename) is additive + DORMANT and live. Full payload = **🟢 CURRENT STATE** at the top of this file.
