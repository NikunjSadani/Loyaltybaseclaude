...
## 🟢 CURRENT STATE
- **🚀 VISIBILITY-LED PAYOUTS: 194C AUTO-INVOICING + CONFIGURABLE TDS — ✅ LIVE ON STAGING + runtime-verified (feature `500eaf9`; develop HEAD now `c2f851a`, 7 ahead of prod `4ebf12c`); ▶ NEXT = owner UAT → owner-gated prod cutover.** *(Also riding develop/staging + the same next cutover: the 🔒 GIFSY assume-tenant scoping fix `706efd1`+`12045db` — memory [[gifsy-assume-tenant-scoping]] — DONE + runtime-proven. See the START-THE-SESSION restart prompt at the bottom for the full current-state + the owner's next-choice menu.)** Pushed develop→staging: both services `500eaf9`, `/health/ready` db:up, BOTH migrations applied to `gifsy_staging` (foundation + credit-code-per-tenant; guarded read confirmed 3 tables + 4 CreditPayoutEntry cols + 3 unique idx). API-surface verified LIVE (config PUT/GET round-trip persists distinct DEDUCT/GROSS_UP; reports+gst-reimbursements 200 w/ {paise,inr}; RBAC negatives 403; Deoleo login 200 byte-identical). Deoleo staging tdsPolicy=194C GROSS_UP (armed for UAT; no-op until a VISIBILITY payout flows). **REMAINING = owner UAT** (per-role click-through + real payout-Excel→confirm→invoice→ledger→UTR-lock write-path E2E) **→ W4 prod cutover.** Full build: W0 schema/migration + W0.5 contracts + W1 backend (dual+dual+final money-audit CLEAN) + W2 FE (5 admin pages under "TDS & Invoicing" nav + admin/invoices tenant-read-only + partner-copy D13/D14 fix). Gate: api nest 0/jest 2023 · FE tsc 0/vitest 2039. **DEFERRED DECISIONS (owner-parked) in WAVE0-SCHEMA §10:** DD-1 (owner "keep as-is" 2026-07-29) tenant recovery report exposes cross-tenant PAN aggregate (`panBase`/`panTdsTotal`) → a tenant can infer amounts a SHARED-PAN retailer got from OTHER 194C tenants (amounts only, no identity; dormant till 2nd 194C tenant); **✅ DD-2 double-reversal + DD-3 no-PAN-per-(client,outlet) + MIXED cross-tenant methodology EXERCISED + CLOSED 2026-07-29** (synthetic 2-tenant staging run, real API full write-path, then fully deleted; numbers paise-exact); **DD-4 (gross-up TDS invoice GST) ✅ CONFIRMED CORRECT AS DESIGNED** (owner-confirmed: unregistered→no GST per D6, registered→GST per D-i; verified live).
...
- **🚀 VISIBILITY (POSM) — ✅ LIVE IN PROD — cutover #16 `4ebf12c` (2026-07-28), DORMANT + post-cutover infra DONE.**
  ...
  applies). **ONLY OPEN = owner activates a tenant (`visibilityConfig`) + UATs in prod.** develop `0615af2` (infra) ahead of main
  by 1 (goes to prod next cutover; prod secret already bound). *(prior: LIVE ON STAGING `6e3b897`.)*
...
> **⏳ PENDING #17 (owner-gated, NOT done):** develop `ab61b63` is **11 commits ahead** of prod `4ebf12c` — the next cutover ships, together: POSM post-cutover infra (`0615af2`), the VISIBILITY-LED-PAYOUTS/194C-TDS feature (`500eaf9`, **+2 additive migrations** `20260728120000_visibility_payout_tds_foundation` + `20260728130000_credit_code_per_tenant_unique`), the GIFSY assume-tenant scoping fix (`706efd1`+`12045db`), the TDS DD-2/3/4 staging-proof docs (`e759e01`+`e784f9a`), and the **PARTNER→MULTI-OUTLET admin grouping FE** (`ab61b63` — the admin UI that #14 shipped a backend for but never a frontend; parents-management page + Parent ID upload column + un-group/deactivate; **NO new migration**). All LIVE + staging-verified. Gate at HEAD: api nest 0 / jest 2067 · FE tsc 0 / vitest 2047. Awaiting owner UAT → merge→main → owner-approved gate.
...
**🟢 THREE deliverables sit on `develop`/STAGING, NOTHING on prod — all ride the NEXT owner-gated prod cutover. develop = origin/develop = `ab61b63` (11 commits ahead of prod `main` = `4ebf12c`).** *(1) Visibility-Led-Payouts/194C-TDS `500eaf9` (+2 migrations); (2) GIFSY assume-tenant scoping `706efd1`+`12045db`; (3) 🆕 **PARTNER→MULTI-OUTLET admin grouping FE** `ab61b63` — the admin UI #14 never built (parents-management page + Parent ID upload column + un-group/deactivate/group-view + backend slice `GET/deactivate /admin/parents/:id` + outlet-list `?parentId`); additive, no migration, dormant-safe, gate-green (api jest 2067 · FE vitest 2047), adversarial-audited (3 fixes; blank-cell no-op proven safe), staging-runtime-verified end-to-end; memory [[partner-multi-outlet]] §"ADMIN-FE CORRECTION" + PARTNER-MULTI-OUTLET.md §10.*

**1) 🚀 VISIBILITY-LED PAYOUTS: 194C AUTO-INVOICING + CONFIGURABLE TDS — ✅ LIVE ON STAGING + runtime-verified (feature commit `500eaf9`).** Full build W0–W2 + **dual money-audit (8)→fix→dual re-audit (4)→fix cycle 2 (6)→FINAL re-audit CLEAN (0)**. BOTH migrations (`20260728120000_visibility_payout_tds_foundation` + `20260728130000_credit_code_per_tenant_unique`) APPLIED to `gifsy_staging` (guarded-read verified: 3 tables + cols + indexes). API-surface verified LIVE (config round-trip persists distinct DEDUCT/GROSS_UP; 3 reports + gst-reimbursement 200 `{paise,inr}`; RBAC negatives 403; Deoleo login 200 byte-identical). Owner decisions: config=JSON-hardened+freeze-on-confirm · gross-up=MONTHLY-INCREMENTAL top-ups · no-PAN=pay-full+20%-tenant-recovery+report · default-ON · D10 pro-rata-by-FY-aggregate. Deoleo staging `tdsPolicy=194C GROSS_UP` (armed; engine no-op until a VISIBILITY payout flows — Deoleo's live incentive/194R path byte-identical). Docs `VISIBILITY-PAYOUT-TDS-INVOICING-DESIGN.md` + as-built/fix-log/§10-DEFERRED-DECISIONS `VISIBILITY-PAYOUT-TDS-WAVE0-SCHEMA.md` + memory [[visibility-payout-tds-invoicing]] — **READ FIRST.** Gate: api nest 0 / jest 2056 · FE tsc 0 / vitest 2039. Money path → **dual audit already done + CLEAN — re-run on ANY further money-logic change.** ▶ REMAINING = **owner UAT** (the one path not exercised: real payout-Excel → confirm → SERVICE invoice + GST-holdback → download/DEDUCT-or-GROSS_UP ledger → UTR-lock; + per-role click-through).

**2) 🔒 GIFSY ASSUME-TENANT SCOPING — ✅ DONE + runtime-proven (`706efd1` + `12045db`).** A GIFSY operator assumed into a tenant is now READ-scoped to that tenant (was platform-wide → owner saw Deoleo's KYC while "working in" a fresh 2nd tenant `uatbajaj`; **NOT a leak** — real tenant admins were always hard-scoped, verified). Invariant `api/src/common/tenant-scope.ts` `platformWide(user)=role==='GIFSY_ADMIN' && !assumed`; 194C platform views are un-assumed-GIFSY-only. Adversarial-audited (core sound, no tenant leak, writes intact) + proven live (assumed→Bajaj-only/empty, un-assumed→all, Deoleo→own; 194c/export/liability 403-when-assumed; unregistered/gst-reimbursement can't be steered to a sibling). Memory [[gifsy-assume-tenant-scoping]] + RESUME trap (ASSUME-TENANT-SCOPE).

**▶ NEXT — owner's choice (do NOT hard-lead):**
- **(a) Owner UAT on staging** — Deoleo admin `6289864191` / Gifsy `9830011252`, OTP `123456`; the new **"TDS & Invoicing"** nav. The full payout-Excel write path is the one thing only a real UAT (or a synthetic proof) exercises live.
- **(b) I seed a synthetic 2nd 194C tenant on staging + drive the full write-path + cross-tenant 194C proof myself** (owner inherits a fully-exercised, isolation-proven system before UAT). *OFFERED — owner to confirm.*
- **(c) Owner-gated prod cutover** — FF merge develop→main; carries the **11-commit bundle**: POSM post-cutover infra (`0615af2` — VISIBILITY_REMINDER_SECRET + scheduler + GCS lifecycle now reach the prod deploy workflows), the TDS feature + **2 ADDITIVE migrations**, the assume-tenant scoping fix, and the **partner→multi-outlet admin grouping FE** (`ab61b63`, no migration). Prod pre-check: the only migrations are the 2 additive TDS ones (additive-safe); post-cutover verify serving SHA + `/health/ready` + a TDS config round-trip + the assumed-scope behaviour + the new `/admin/users/parents` page loads.
