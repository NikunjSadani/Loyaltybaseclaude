...
**🟢 EVERYTHING SHIPPED IS LIVE IN PROD — CUTOVER #19 (`daa4f3f`, 2026-07-30), verified.** prod = main = `daa4f3f`; develop = `b1e059a` (docs ahead — always verify via `git log`, don't trust a hardcoded SHA). Recent cutovers: **#19** scheme roster-upload Excel report · **#18** scheme prefill Editable/Locked · **#17** TDS/194C + assume-tenant scoping + partner-grouping FE. All additive + DORMANT.

## ▶▶ START HERE (post-compact) — SCHEME UX HARDENING BUILD (planned + decisions locked; NOT started)
**READ FIRST: `platform/docs/plans/SCHEME-UX-HARDENING-PLAN.md`** — full 4-slice review of the LIVE scheme feature consolidated into a concrete, parallelized, **CODE-ONLY (no migration)** build. The core engine is sound; gaps are in authoring UX + reviewability.
...
- **Dual prefill source (the headline new capability + the fix to "how do I link a variable"):** the form-builder prefill link becomes a **DROPDOWN of real values, not blind free-text** — pick from the **actual uploaded roster columns** OR an **Outlet master field** (Owner Name/Phone/PAN/Address/Program…). For a **matched loyalty outlet** an Outlet-field auto-fills from its existing DB record; for a **standalone/non-loyalty** outlet the Excel column fills it (set both = outlet-field-for-loyalty-else-Excel = the MIXED auto-fill). Also fixes the integrity bug where a typo in the old free-text `prefillKey` silently *unlocked* a Locked field.
- **🔴 must-fixes:** blind free-text prefill link → dropdown (H1) · DATA_DISPLAY value stripped (`collectPrefillKeys` ignores `dataDisplayKey`) (H2) · scheme activatable with no audience/form (H3) · Save not gated on builder validation → raw 400s (H4) · admin detail drawer reads wrong shape (outlet name/code blank, geo cross-check inert) (H5) · signature capture distorted (unscaled canvas) (H6). + ~14 🟡 should-fix (validator parity · reject audit-trail lost · broadcast has no recipient-count confirm [billable] · approved-outlet phone pre-pin · filter-facet picker · GPS accuracy D15 · roster template download · calculated preview · export media-URL/TOGGLE · scoping unify) + ⚪ polish batch.
- **Execution:** Wave 0 = backend contracts (`GET :id/roster/columns` + facet-values + outlet-field catalog + dual-source resolution on the 3 enroll payloads + the backend must-fixes + remove `FormField.audience`) → Wave 1 = 4 parallel file-disjoint FE streams (Builder · Renderer · Authoring flow · Reports) → integrate → full gate → **dual adversarial audit** (prefill/consent path) → staging runtime-verify (a MIXED matched+standalone roster proving dual-source prefill + lock) → docs/memory sweep → **owner-gated cutover**. **NEXT ACTION = start Wave 0.**

## ▶ ALSO PICK UP (post-compact) — PARENT ID missing from outlet-master DOWNLOAD
The **"Parent ID"** column was added to the outlet-master **UPLOAD** at cutover #17 (`platform/src/lib/outlet-upload.ts` — optional header, parsed, in template + guide) but is **NOT in the outlet-master DOWNLOAD/export** (`platform/src/lib/outlet-master-export.ts` `COLUMNS` has no Parent ID; the backend export row source must also project the outlet's parent code). **Impact:** an admin can't SEE or round-trip (download → edit → re-upload) the owner-group linkage via the master export. **Fix:** add a "Parent ID" column to the export — FE `COLUMNS` in `outlet-master-export.ts` + backend export projection of the outlet's `parentCode`/`parentId`. Additive, code-only. Memory [[partner-multi-outlet]].
