...
**PARKED (2nd-tenant checklist / CA-review — NOT blockers):** DD-1 tenant recovery report exposes the cross-tenant PAN aggregate (`panBase`/`panTdsTotal`) to a tenant admin — **owner KEEP-AS-IS** (fix = strip those 2 fields when scope.clientId set); DD-2 low-sev double-reversal edge; DD-3 no-PAN threshold per-(clientId,outletCode) + MIXED cross-tenant methodology (handled, unexercised) → CA review — all in WAVE0-SCHEMA §10. (batchCode/downloadCode global-unique collision was FIXED this wave. DB-level RLS still backlog Gap #23 — isolation is application-`clientId`-scoped.)
