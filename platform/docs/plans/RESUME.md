...
⚠️⚠️ **PHASE S — BACKEND SPLIT (API-first) — IN PROGRESS. S0–S7 DONE; NEXT = S8 (cutover).** DECIDED by owner
...
**NEXT = S8 (cutover, human-gated):** end-to-end smokes (web→backend→DB) + delete World-A `api/` leftovers (old seed etc.) **+ delete the now-shadowed local `src/app/api/*` ported route handlers** (keep the 4 deferred groups — rewards/redeem, visibility/submit, partner/invoices, admin/kyc — until ported; after deleting platform's ported routes, platform's Prisma schema can go too → it becomes a pure proxy FE) + close #30/#31.
