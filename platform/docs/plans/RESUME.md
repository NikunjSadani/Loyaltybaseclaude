...
⚠️⚠️ **PHASE S — BACKEND SPLIT (API-first) — IN PROGRESS. S0–S6 DONE; NEXT = S7 → S8 (light).** DECIDED by owner
...
`docs/plans/BACKEND-SPLIT-PLAN.md` (steps S0–S8). **S0–S6 ✅ DONE on `develop` (S0–S5 pushed; S6 commit-pending; run `git log --oneline -30`):**
...
**NEXT = S7–S8 (light):** **S7** infra/CI near-no-op — **set `NEXT_PUBLIC_API_URL` (backend origin) on `gifsy-frontend` in prod/staging** (terraform/CI); drop the dead `prisma generate --schema=../api/prisma/...` fallback · **S8** cutover smokes (web→backend→DB) + delete World-A `api/` leftovers (old seed etc.) **+ delete the now-shadowed local `src/app/api/*` ported route handlers** (keep the 4 deferred groups until ported) + close #30/#31.
