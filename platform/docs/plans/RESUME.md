...
- **⚠ Residual (known, low):** `deactivate()` still throws a 400 on a zero-match (a deliberate destructive-path guard), so a >500-code deactivate whose LAST batch is all-invalid would abort + discard earlier batches. Very narrow (the FE only sends existing+active codes; a tail batch going all-invalid needs a concurrent deactivation). Left as-is; revisit if it ever bites.
