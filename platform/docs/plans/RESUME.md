...
- **✅ CUTOVER #21 LIVE + VERIFIED (`a83b2f4`, 2026-07-31) — prod = main = origin/develop = `a83b2f4` (develop is NO LONGER ahead of prod).** Verify HEADs via `git log`. Payload = 16 commits `8c08af3..a83b2f4`, **ALL additive + DORMANT** (Deoleo live path byte-identical until a Gifsy admin uses these features). **Gate at cutover: api build 0 · jest 2124 · FE tsc 0 · vitest 2052.** Migration `20260730160000_scheme_enrollment_soft_delete` **APPLIED to prod** (guarded read: `done:true`, `rolled:false`, `scheme_enrollments.deletedAt` nullable col + `scheme_enrollments_deletedAt_idx` present; already on gifsy_staging). Prod-verified: both services `a83b2f4`, `/health/ready` db:up, `/v1/schemes` + `/v1/schemes/:id/enrollments/deleted` + `/v1/admin/settings` all 401-wired, `deoleoloyalty.gifsy.in/auth/login` 200. Rollback ref `8c08af3` (#20). *(See ledger row #21.)* The 6-part payload:
  ...
  - **IDENTITY & PAYOUT UNIQUENESS SETTINGS TOGGLE** (`a83b2f4`): a self-serve Admin → Settings card for the per-tenant `uniquenessPolicy` (PAN/GST always-on+locked; Mobile/Bank/UPI editable OFF/ON), GIFSY_ADMIN-only. ⚠️ **default bank/UPI OFF; PROD deoleo policy UNCHANGED (bank/UPI still OFF)** — staging was flipped all-on after a dup-sweep; the owner flips prod ON via this toggle (OPEN owner action, NOT done). See [[global-settings-wiring]].
...
- **prod SERVES `a83b2f4` (cutover #21, 2026-07-31); main == origin/develop == `a83b2f4` (develop NOT ahead).** *(history: `8c08af3` #20, `daa4f3f` #19, `f193127` #18, `52fc19f` #17.)* Cutover #21 = **scheme UAT batches + child-KYC group-identity prefill + Identity/Payout Uniqueness toggle** (carries the `scheme_enrollment_soft_delete` migration, applied to prod; see 🟢 CURRENT STATE + ledger row #21). Cutover #20 = **scheme UX-hardening + Parent-ID outlet-master export** (code-only; see ledger row #20). Cutover #19 = **scheme roster-upload Excel report** (Download-report button → Summary + per-row disposition + Duplicates + Unmatched-Employees sheets; code-only, GIFSY-admin-only). Cutover #18 = **scheme prefill Editable/Locked** (backend-enforced Excel-variable prefill; memory [[scheme-data-collection]] §16).
...
**✅ CUTOVER #21 LIVE + VERIFIED (2026-07-31) — prod = main = origin/develop = `a83b2f4` (develop is NOT ahead of prod)** (always verify via `git log`, don't trust a hardcoded SHA). Shipped (16 commits `8c08af3..a83b2f4`, ALL additive + DORMANT): **Batch 1** scheme UAT fixes · **Batch 2** edit/delete a filled enrollment (carries the applied-to-prod `scheme_enrollment_soft_delete` migration) · scheme sales-list scoping · enrollment Excel-report rebuild · **child-KYC group-identity prefill** · **Identity & Payout Uniqueness Settings toggle**. Gate: api build 0 · jest 2124 · FE tsc 0 · vitest 2052. There is NO pending scheme batch — develop == main. Recent cutovers: **#21** UAT batches + child-KYC prefill + uniqueness toggle · **#20** scheme UX-hardening + Parent-ID export · **#19** scheme roster-upload Excel report · **#18** scheme prefill Editable/Locked · **#17** TDS/194C + assume-tenant scoping + partner-grouping FE. All additive + DORMANT. **OPEN owner items (their choice among the leftovers): post-#21 real-phone smoke · the bank/UPI prod uniqueness-policy flip (prod still OFF) · UAT of the older dormant surfaces (TDS payout write-path, grouping UI, visibility POSM, scheme).**

## ▶▶ START HERE (post-compact) — CLEAN STATE: cutover #21 is LIVE; develop == main; no pending build
**prod = main = origin/develop = `a83b2f4` (cutover #21 LIVE, 2026-07-31).** Verify HEADs via `git log` — there is NO scheme batch waiting on staging; develop is NOT ahead of prod. Everything from cutover #21 (the 6-part payload — scheme UAT batches, edit/delete-a-filled-enrollment with the applied-to-prod `deletedAt` migration, sales-list scoping, Excel-report rebuild, child-KYC group-identity prefill, uniqueness toggle) is additive + DORMANT and live. Full payload = **🟢 CURRENT STATE** at the top of this file.
...
- **Post-#21 real-phone smoke** — the one device path not verifiable without a handset: camera-only live capture (getUserMedia), geo, signature canvas, and a live scheme enroll on a real phone. ~5–10 min.
- **Bank/UPI prod uniqueness-policy flip** — the new Identity & Payout Uniqueness toggle is live, but **PROD deoleo `uniquenessPolicy` bank/UPI is still OFF** (staging was flipped all-on after a dup-sweep found 2 test-data bank dups, 0 UPI). Flipping prod ON is an OPEN owner action — do a dup bank/UPI sweep FIRST (no DB index reveals them), then the owner turns it on via the toggle (or asks). PAN/GST stay always-on regardless.
- **UAT of the older dormant surfaces (no rush; nothing blocks Deoleo):** TDS payout write-path (#17) · the `/admin/users/parents` grouping UI (#17) · VISIBILITY POSM `visibilityConfig` (#16) · the SCHEME instrument + UX-hardening (#15/#18/#19/#20/#21) — all covered in the sections below.
...
- **OPEN owner items:** post-#21 real-phone smoke · the bank/UPI prod-policy flip (prod still OFF). Rollback ref `8c08af3` (#20).
