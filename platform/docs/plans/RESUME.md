...
- **prod SERVES `daa4f3f` (cutover #19, 2026-07-30); main == `daa4f3f`.** *(history: `f193127` #18, `52fc19f` #17.)* Cutover #19 = **scheme roster-upload Excel report** (Download-report button on the roster upload result → Summary + per-row disposition + Duplicates + Unmatched-Employees sheets; code-only, no migration; GIFSY-admin-only; audited GO + staging-verified). Cutover #18 = **scheme prefill Editable/Locked** (backend-enforced Excel-variable prefill; memory [[scheme-data-collection]] §16).
...
**⏳ `develop` IS 5 COMMITS AHEAD OF PROD, STAGING-VERIFIED, AWAITING CUTOVER (pending #20).** prod = main = `daa4f3f` (cutover #19); develop = `273a4b2` (serving on staging — always verify via `git log`, don't trust a hardcoded SHA). The 5 commits = **SCHEME UX-HARDENING** (dual-source prefill + H1–H6; dual-audited + staging-verified 21/21; code-only, no migration) + the **PARENT ID outlet-master DOWNLOAD** fix — awaiting an owner-gated cutover (see START HERE below). Recent cutovers: **#19** scheme roster-upload Excel report · **#18** scheme prefill Editable/Locked · **#17** TDS/194C + assume-tenant scoping + partner-grouping FE. All additive + DORMANT.

## ▶▶ START HERE (post-compact) — SCHEME UX HARDENING BUILD — ✅ BUILT + STAGING-VERIFIED (21/21), awaiting owner cutover
**READ FIRST: `platform/docs/plans/SCHEME-UX-HARDENING-PLAN.md`** (now carries the ✅ status banner + row/phase annotations) — full 4-slice review of the LIVE scheme feature, built as a concrete, parallelized, **CODE-ONLY (no migration)** change. The core engine was sound; gaps were authoring UX + reviewability. **✅ BUILT + DUAL-AUDITED + STAGING-VERIFIED (21/21) on `develop` 273a4b2 — awaiting owner-gated prod cutover (pending #20).**
...
- **Dual prefill source (the headline new capability + the fix to "how do I link a variable"):** the form-builder prefill link is now a **DROPDOWN of real values, not blind free-text** — pick from the **actual uploaded roster columns** OR an **Outlet master field** (Owner Name/Phone/PAN/Address/Program…). Contract = `FormField.outletField` resolves from a matched loyalty outlet's DB record; else `prefillKey` Excel column; server-authoritative pin for locked fields (outlet-field WINS over a same-named Excel column). Also fixed the integrity bug where a typo in the old free-text `prefillKey` silently *unlocked* a Locked field.
- **🔴 must-fixes — ✅ all done:** blind free-text prefill link → dropdown (H1) · DATA_DISPLAY value no longer stripped (H2) · activation requires audience + non-empty form (H3) · Save gated on builder validation (H4) · admin detail drawer nested outlet/geo reads + id→label map (H5) · signature-canvas scaling fix (H6). + the ~14 🟡 should-fix + ⚪ polish batch all shipped (validator parity · immutable REJECTED-submission reject-trail · billable-broadcast preview+confirm · approved-owner phone pre-pin · facet-values picker · GPS-accuracy cap D15 · roster template download · calculated preview · export TOGGLE→Yes/No + app-proxy media URL · report/notify `platformWide` scoping-unify · removed `FormField.audience`).
- **New reads:** `GET :id/prefill-sources` (real Excel cols + 15 outlet fields) · `:id/facet-values` · broadcast dry-run `:id/broadcast/preview`.
- **Gate green:** api jest 2104 · nest 0 · FE tsc 0 · vitest 2049. **Dual adversarial audit:** backend (tenant-isolation/consent/prefill CLEAN + 4 lower-sev fixes) AND FE (no HIGH; 2 fixes — activation gate reads PERSISTED form not draft, dual-source merge lets outlet-field WIN). **STAGING RUNTIME-VERIFIED 21/21** (synthetic MIXED matched+standalone roster, then guarded soft-deleted): dual-source PIN proven (rep sent f_owner="HACKED-BY-REP" → server stored the DB owner name), editable prefill kept the rep value, rep payload no PAN/GST leak, reject-trail REJECTED:v2 after SUBMITTED:v1, broadcast preview count.
- **Commits (develop, pushed):** c43304e (W0 backend) · 0da1859 (W0 audit fixes + FE contract) · ed4a577 (Parent ID export) · 16358a7 (W1 FE) · 273a4b2 (W1 FE audit fixes). develop HEAD = 273a4b2; main/prod still `daa4f3f`.
- **NEXT ACTION = owner-gated cutover** (merge develop→main + approve the prod "Deploy — Production" gate) **+ a ~5–10 min real-phone smoke** (signature canvas, camera/geo, dual-source locked prefill on a live enroll).

## ▶ ALSO PICK UP — PARENT ID in outlet-master DOWNLOAD — ✅ DONE (staging-verified on develop 273a4b2, awaiting cutover)
The **"Parent ID"** column was added to the outlet-master **UPLOAD** at cutover #17 but was missing from the **DOWNLOAD/export**. **✅ FIXED (commit ed4a577, code-only):** the real export is the **backend** path `reports.service.outletMaster` — it now emits a "Parent ID" column (the outlet's parent ChannelPartner `partnerCode`), **57 columns total**, round-tripping the upload's "Parent ID". The FE `platform/src/lib/outlet-master-export.ts` the backlog named is **DEAD/unused demo code** (not the real export). Additive, no migration. Memory [[partner-multi-outlet]].
