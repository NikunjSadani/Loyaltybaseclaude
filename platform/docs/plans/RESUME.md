...
## ▶▶ START HERE (post-compact) — NEXT UP: the UI/UX AUDIT FIX PLAN (owner said "we start this entire thing after compact")
**prod = main = `fb996d8` (cutover #22 LIVE, 2026-07-31). develop = `825af2f` — AHEAD of prod by FOUR built+verified items + docs.** Verify HEADs via `git log`. Gate on develop: **api jest 2177 (+18 geo-fence spec) · nest 0 · FE tsc 0 · vitest 2066.**

### ▶▶ THE QUEUED WORK (owner-directed, start FIRST after compact): UI/UX AUDIT FIX PLAN
**Owner: "we will start with this entire thing after compact."** The new **mandatory UI/UX audit lane** ([[audit-every-build-item]]) was run retroactively over recently-shipped FE (4 independent auditors) → **52 findings** triaged + estimated in **`platform/docs/plans/UIUX-AUDIT-FIXPLAN.md`** (READ IT FIRST — full per-item table + phasing). Rollup:
- **P1 — roster-remove polish (8 items, ~1.3d)** — NOT yet in prod → fix on develop so it rides cutover #23 (RST-H1 pre-commit affected-count · RST-H2 "removed stays removed" copy · RST-M1 restore-panel pagination · RST-M2 double-upload race · L1–L4).
- **P2 — prod-live HIGH bugs (10, ~2.6d)** — real defects: **SIG-1** KYC signature draws OFFSET from finger on phone (canvas not scaled) · **SIG-2** scheme-enroll signature needs a separate "Save" tap → draw→Submit silently loses it · **SET-H2** uniqueness Settings stale-cache + REPLACE-WHOLE save can silently flip a sibling toggle off (KYC money path) · **ENR-H3** phone editable after OTP → verify code-A vs number-B · **ENR-H4** form-fetch blip → "formless" enroll capturing zero fields · **KYC-H1** disabled Continue no visible reason · **KYC-H4** consent checkboxes non-semantic divs · **ENR-M1/KYC-up** submit-while-upload race · **KYC-H2** geo hard-block no retry (see D1).
- **P3 — prod-live MED (14, ~2.3d)** · **P4 — LOW/a11y (16, ~2.0d)** · **G1 — geotag polish (3, ~0.4d)**. **Total ≈ 12 engineer-days orchestrated.**
- **⚠️ D1 — OWNER DECISION NEEDED before P2 finalizes KYC-H2:** the KYC geo capture is a HARD block (a rep with device location off / no indoor fix CANNOT submit). Keep hard-block + add a "Retry location" button, OR switch to **flag-and-allow** (capture unverifiable, approver judges) to match the geotag `GEO_UNVERIFIABLE` direction. **Ask the owner first.**
- **Recommended sequencing (owner to confirm):** P1 now (cheap, rides #23) → P2 as its own audited cutover (fold in P3 + G1) → P4 opportunistically. Each phase = orchestrate → full gate → INDEPENDENT audit (dual for KYC/consent/money-settings) → staging-verify + owner device-smoke (camera/GPS/signature). **P2's signature/geo/consent/settings paths → mandatory dual audit.**

### ▶ ALSO STACKED ON develop, awaiting owner-gated CUTOVER #23 (built + audited + staging-verified):
1. **SCHEME ROSTER-ROW REMOVE** (`378f795`, SchemeOutlet soft-delete; "removed stays removed"; triple-audited; staging 16/16). **Carries additive migration `20260801120000_scheme_outlet_soft_delete`** — **applied to gifsy_staging, NOT prod** (applies at cutover via deploy.yml). Design `SCHEME-ROSTER-ROW-REMOVE-PLAN.md`. ⚠️ P1 UX fixes above should land on this BEFORE #23.
2. **AUTH OPTION B — check-first login OTP** (`6ba9ead`) — `sendOtp` refuses "No account found" if the tenant-scoped number is unregistered; login-only, no FE/migration; dual-audited; staging 5/5.
3. **KYC GROUPED-CHILD SHARED-PHONE FIX** (`e0f767b`) — the sales new-KYC mobile-conflict check is now GROUP-AWARE (a phone shared with a SAME-GROUP sibling is NOT flagged, mirroring backend `assertPhoneAvailable`; `sales.service` returns `Outlet.parentId`). Fixes the owner-reported "subsequent grouped outlet can't continue KYC" — the tester was blocked at the MOBILE field (shared owner phone `7755889900`), NOT the GST doc. FE-only, no migration; audited clean; staging data-verified.
4. **PER-PHOTO GEOTAG** (`ee00dd2` build + `21fe2c7` UX fixes) — each CAMERA photo captures its OWN GPS at shutter; per-photo visibility geo-fence; replaces the never-wired `ON_PHOTO` label. NO migration (geo in `formValues` JSON). Backend anti-fraud DUAL-audited (2 HIGH+2 LOW fixed: geoFenceOk=true only genuinely-verified-inside; tokenized report link no longer 404s geotagged photos) + FE UI/UX audited (2 HIGH fixed: visibility double-capture race + silent-no-location hint). New `geo-fence.helper.ts` (18-test spec). Design `PER-PHOTO-GEOTAG-PLAN.md`; memory [[visibility-posm]]. ⚠️ residual: owner keep-flag-vs-hard-block on a no-fix fence-required photo (1-line) + owner ~10-min device smoke (camera/GPS/signature).
