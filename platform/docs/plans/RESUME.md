...
## 🟢 CURRENT STATE
- **🔧 DEVELOP IS AHEAD OF PROD (#20) WITH TWO ADDITIVE + DORMANT SCHEME BATCHES (post-#20 owner UAT).** Verify HEADs via `git log`. **local develop = `c807291`** (one WIP commit AHEAD of origin/develop, NOT pushed) · **origin/develop = `c510cc0`** (pushed, LIVE ON STAGING) · **prod = main = `8c08af3`** (#20). Both batches are additive + dormant (Deoleo live path byte-identical until a Gifsy admin uses a scheme with these features).
  ...
  - **BATCH 2 — "C: edit/delete a filled enrollment" (WIP `c807291`) — BUILT + INTEGRATED, LOCAL ONLY (not pushed), NOT fully gated / NOT audited / NOT staging-verified. CARRIES A MIGRATION.** Owner decisions (locked): admin edit → saves as a NEW version (append, trail kept); enroller edit → allowed ONLY when the admin turns on a per-scheme flag `audienceConfig.allowEnrollerEdit`; delete → soft-delete that FREES the roster row to re-enroll (recoverable). Design (avoids a risky change to the enrollment 1:1-unique): additive `SchemeEnrollment.deletedAt` column + "reset-on-reenroll" — delete sets `deletedAt` (hidden everywhere); re-enrolling that outlet RESETS the same row (clears `deletedAt` + appends a version); history retained; recoverable. **Migration** `20260730160000_scheme_enrollment_soft_delete` (additive `deletedAt` + `@@index([deletedAt])`) — WRITTEN, prisma client regenerated, **NOT applied to any DB.** Backend (scheme-enrollment.service + controller + scheme-admin.service + scheme-report.service + dto): admin edit `PUT :id/enrollments/:enrollmentId` (→ new version) · `DELETE :id/enrollments/:enrollmentId` (soft-delete) + `POST :id/enrollments/:enrollmentId/restore` · `enroll()` re-enrolls a soft-deleted row (resets it) · `resubmit()` also allows a SUBMITTED enrollment when `allowEnrollerEdit` · hide-soft-deleted in EVERY read/export (getMyEnrollment/getSalesTargets/adminList/adminGet/getRoster/loadAggregation/exportEnrollments/getRosterExport) · `SetAudienceDto.allowEnrollerEdit`. FE (admin enrollments page + SchemeAudienceEditor + partner/schemes/[id] + sales SchemeEnrollSheet + lib/schemes.ts): admin drawer **Edit / Delete / Undo** · **"Allow enroller to edit their own submission"** audience toggle · **partner** self-edit WORKS (resubmit, pre-fills full prior values) · **SALES-rep self-edit is DISABLED** (`allowEnrollerEdit=false` hardcoded in SchemeEnrollSheet — the sales sheet re-opens with roster PREFILL only, no prior values, so an edit there would risk a blank overwrite; a GIFSY admin CAN edit sales-filled enrollments; a safe rep-side edit needs a per-target formValues source — DEFERRED as an offered follow-up). schemeApi: adminEditEnrollment / deleteEnrollment / restoreEnrollment + SetAudienceInput.allowEnrollerEdit. Gate so far: api nest build 0 / jest 2107 / FE tsc 0. **FE vitest NOT run (owner interrupted it).** **REMAINING for Batch 2 = the NEXT-ACTION list in ▶▶ START HERE below (run FE vitest → dual-audit → push → apply staging migration → staging-verify → cutover #21).**
...
> **📌 PENDING CUTOVER #21 (Batch 1 + Batch 2 — CARRIES a migration).** Not yet cut. **Batch 1** ("UAT fixes" — reach fix, camera-only capture, roster view/download .xlsx, DATA_DISPLAY dropdown; `c510cc0`) is LIVE ON STAGING, code-only, gate-green + audited CLEAN, awaiting the owner's staging re-test. **Batch 2** ("C: edit/delete a filled enrollment"; WIP local `c807291`, NOT pushed) carries the **additive** migration `20260730160000_scheme_enrollment_soft_delete` (`SchemeEnrollment.deletedAt` + index) → prod pre-check + `migrate deploy` (NOT code-only). Both additive + DORMANT. Remaining before #21: Batch-2 FE vitest → dual-audit → push → apply staging migration → staging-verify → owner-gated cutover. Rollback ref = `8c08af3`. *(See ▶▶ START HERE.)*
>
...
**✅ CUTOVER #20 LIVE + VERIFIED (2026-07-30) — prod = main = `8c08af3`** (always verify via `git log`, don't trust a hardcoded SHA). Shipped = **SCHEME UX-HARDENING** (dual-source prefill + H1–H6; dual-audited BE+FE + staging-verified 21/21; code-only, no migration) + the **PARENT ID outlet-master DOWNLOAD** export fix. **⚠️ DEVELOP IS AHEAD OF PROD** with TWO post-#20 owner-UAT batches (both additive + DORMANT): **Batch 1** (UAT fixes — reach fix, camera-only capture, roster view/download, DATA_DISPLAY dropdown) is **LIVE ON STAGING** (origin/develop `c510cc0`, gate-green + audited CLEAN, code-only) awaiting the owner's staging re-test; **Batch 2** (C: edit/delete a filled enrollment) is **WIP LOCAL ONLY** (`c807291`, NOT pushed, NOT fully gated/audited/verified, **carries a migration**). **NEXT MOVE = the Batch-2 NEXT-ACTION list in ▶▶ START HERE below.** Recent cutovers: **#20** scheme UX-hardening + Parent-ID export · **#19** scheme roster-upload Excel report · **#18** scheme prefill Editable/Locked · **#17** TDS/194C + assume-tenant scoping + partner-grouping FE. All additive + DORMANT.

## ▶▶ START HERE (post-compact) — RESUME EXACTLY HERE: finish BATCH 2 (C edit/delete enrollment) — WIP LOCAL ONLY (`c807291`), carries a migration
**READ FIRST: memory [[scheme-data-collection]] + `platform/docs/plans/SCHEME-DATA-COLLECTION-DESIGN.md`.** This session (post-#20) surfaced two owner-UAT batches. **Batch 1 is on staging awaiting the owner's re-test; Batch 2 is the active build to finish — pick up at step (1) below.** Full batch contents are in **🟢 CURRENT STATE** at the top of this file. Verify HEADs via `git log` (local develop `c807291` · origin/develop `c510cc0` · prod `8c08af3`).

### The active build = BATCH 2 — "C: edit/delete a filled enrollment" (WIP `c807291`, LOCAL ONLY, carries the `deletedAt` migration)
**Owner decisions (locked):** admin edit → NEW version (append, trail kept) · enroller edit → allowed ONLY when the admin turns on per-scheme `audienceConfig.allowEnrollerEdit` · delete → soft-delete that FREES the roster row to re-enroll (recoverable). **Design:** additive `SchemeEnrollment.deletedAt` + "reset-on-reenroll" (delete sets `deletedAt`/hidden everywhere; re-enroll RESETS the same row + appends a version; history retained, recoverable) — avoids a risky change to the enrollment 1:1-unique. **SALES-rep self-edit is DISABLED** (`allowEnrollerEdit=false` hardcoded in SchemeEnrollSheet — the sales sheet re-opens with roster PREFILL only, no prior values → a rep edit would risk a blank overwrite; a GIFSY admin CAN edit sales-filled enrollments; a safe rep-side edit needs a per-target formValues source — DEFERRED as an offered follow-up). Migration `20260730160000_scheme_enrollment_soft_delete` WRITTEN + prisma client regenerated, **NOT applied to any DB**. Gate so far: api nest 0 / jest 2107 / FE tsc 0; **FE vitest NOT run (owner interrupted it).**
- **▶ NEXT ACTION — do IN ORDER:**
  1. **Run FE vitest** (`cd platform && npx vitest run`) — fix anything.
  2. **DUAL adversarial audit** of the edit/delete write-paths (money/consent-adjacent → MANDATORY): soft-delete/restore auth (GIFSY-only) · reset-on-reenroll can't resurrect another row / lose history · hide-soft-deleted is COMPLETE (no deleted enrollment leaks into any list/report/export/coverage count) · `allowEnrollerEdit` gate can't be bypassed · admin edit appends a version (no silent overwrite). Fix findings.
  3. **Push develop → staging.**
  4. **Apply the migration to `gifsy_staging`** via the in-VPC migrate-deploy Cloud Run Job ([[migration-model]] — NOT from the laptop; private-IP DB). Verify `deletedAt` column present.
  5. **Staging runtime-verify:** delete → row re-enrollable · admin edit → new version · partner `allowEnrollerEdit` edit · a deleted enrollment hidden from admin list + report + roster + export.
  6. **Docs/memory sweep → owner-gated cutover #21** (CARRIES the migration → prod pre-check + `migrate deploy`, NOT code-only like #20). Rollback ref = `8c08af3`.

### Also on the bench = BATCH 1 — "UAT fixes" (`c510cc0`) — LIVE ON STAGING, awaiting the owner's re-test
