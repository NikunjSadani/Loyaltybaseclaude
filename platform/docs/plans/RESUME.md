...
- **prod SERVES `52fc19f` (cutover #17, 2026-07-29); main == develop == `52fc19f`.** *(history: was `bda9bf3` at #15.)*
...
**🟢 ✅ ALL LIVE IN PROD — CUTOVER #17 (`52fc19f`, 2026-07-29), verified. prod = main = develop = `52fc19f`.** The three deliverables shipped together (all additive + DORMANT, Deoleo live path byte-identical): (1) Visibility-Led-Payouts/194C-TDS `500eaf9` (+2 migrations, applied+verified); (2) GIFSY assume-tenant scoping `706efd1`+`12045db` (active); (3) **PARTNER→MULTI-OUTLET admin grouping FE** `ab61b63` (parents-management page + Parent ID upload column + un-group/deactivate/group-view + backend `GET/deactivate /admin/parents/:id` + outlet-list `?parentId`; dormant till an admin sets a parentId); memory [[partner-multi-outlet]] §"ADMIN-FE CORRECTION" + PARTNER-MULTI-OUTLET.md §10. **▶ REMAINING = owner UAT in prod** of the two dormant surfaces: the TDS payout write-path (needs a VISIBILITY payout to flow) + the `/admin/users/parents` grouping UI (create a parent → set its code in the outlet upload's Parent ID column). Gate at cutover: api nest 0 / jest 2067 · FE tsc 0 / vitest 2047.
