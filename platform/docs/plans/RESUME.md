...
**Migrations applied to dev:** `20260622120000_kpi_one_primary_per_client` (pre-compact; U7–U10 are schema-free). **Gate at session end:** api jest **950/950**, FE vitest **1469**, tsc clean both sides; e2e baseline **292/0/8** (a "50-failure" run was the dev Cloud SQL proxy `--token` expiring mid-run — restart proxy + KILL all stale procs first; NOT a defect).
**🔴 OPEN UAT AUDIT FINDINGS (AF-1..AF-11 in `GO-LIVE-ISSUE-LIST.md` → "UAT AUDIT FINDINGS"):** 🔴 fabricated data on LIVE screens (partner dashboard "800 cases", sales dashboard mock `OUTLET_ACHIEVEMENTS`, partner profile HDFC bank — root `lib/targets.ts`/`MOCK_PROFILE`); 🔴 CSV-formula injection on non-finance exports (route through `cellSafe`); 🔴 JWT mirrored to localStorage (XSS-exfil); 🟠 GSTIN 12-char/no-checksum; 🟠 invoice-numbering skip-on-collision; 🟡 hardening cluster. Money·finance·RBAC/tenant/IDOR cores were independently CONFIRMED SOUND.
**NEXT:** (a) owner re-tests on staging (verify serving image SHA = tip `3b4209b`): override column · chain upload · pace view; (b) **fix AF-1..AF-11** (start with the 🔴 fabricated-data + CSV-injection); (c) gap-#57(a) sub-dashboards hide-vs-wire; (d) #76 prod data load → #74 owner ops → go-live.
═══════════════════════════════════════════════════════════════════════════════════════════════════════════════
