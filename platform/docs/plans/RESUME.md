...
**UAT FIXES — 2026-06-23 (all gate-green + PUSHED; full detail = `GO-LIVE-ISSUE-LIST.md` U11–U16):**
...
**Migrations applied to dev:** `20260622120000_kpi_one_primary_per_client` (pre-compact; U7–U16 are schema-free). **Gate (2026-06-23):** api jest **968/968**, FE vitest **1479**, tsc clean both sides. **Staging:** FIXED_OTP=123456 live (verified); login fix + outlet-type fix DEPLOYING (verify serving SHA = `a4852a5`). **Env facts changed this session:** staging OTP now `123456` (was real MSG91); staging deoleo admin phone `6289864191` (GIFSY admin `9830011252`/clientId gifsy; partner `7795096288`; sales `9900000041`=ISR/`9900000002`=SO; the old `9875436349` was parked off its user).
**🔴 OPEN UAT AUDIT FINDINGS (AF-1..AF-11 in `GO-LIVE-ISSUE-LIST.md` → "UAT AUDIT FINDINGS"):** 🔴 fabricated data on LIVE screens (partner dashboard "800 cases", sales dashboard mock `OUTLET_ACHIEVEMENTS`, partner profile HDFC bank); 🔴 CSV-formula injection on non-finance exports; 🔴 JWT mirrored to localStorage; 🟠 GSTIN 12-char/no-checksum; 🟠 invoice-numbering skip-on-collision; 🟡 hardening cluster. Money·finance·RBAC/tenant/IDOR cores CONFIRMED SOUND.
**NEXT (owner-ordered 2026-06-23):** (1) **verify the two staging deploys** once live — a real **sales login** (uat.deoleoloyalty.gifsy.in, phone `9900000041`, OTP `123456`) + outlet-type shows `SSS`; (2) **dev DB stray-`RETAILER` cleanup** (merge into `SSS`; staging/prod already clean); (3) ⭐ **AF-1/AF-2/AF-3 partner+sales FABRICATED DATA — AUDIT-FIRST then fix** (owner: "I remember this was already done #40/#54 — re-derive why it's still mock & missed in audits"; `usePartnerSession` STILL returns `DEMO_SESSIONS`; wire it to `/auth/me` real points+name + `/partner/targets` real primary KPI [needs backend `getTargets` +isPrimary+unit, a stashed-then-dropped edit]; sales `lib/sales-role.ts` personas + DEMO switcher); (4) rest of AF (CSV-injection 🔴 → `cellSafe`, JWT-localStorage 🔴, GSTIN/invoice 🟠); (5) gap-#57(a) sub-dashboards hide-vs-wire; (6) #76 prod data load (MUST route through tenant provisioning so outlet-types/configs exist) → #74 owner ops → go-live.
═══════════════════════════════════════════════════════════════════════════════════════════════════════════════
