...
- **🚀 VISIBILITY-LED PAYOUTS: 194C AUTO-INVOICING + CONFIGURABLE TDS — ✅ LIVE ON STAGING + runtime-verified (feature `500eaf9`; develop HEAD now `c2f851a`, 7 ahead of prod `4ebf12c`); ▶ NEXT = owner UAT → owner-gated prod cutover.** *(Also riding develop/staging + the same next cutover: the 🔒 GIFSY assume-tenant scoping fix `706efd1`+`12045db` — memory [[gifsy-assume-tenant-scoping]] — DONE + runtime-proven. See the START-THE-SESSION restart prompt at the bottom for the full current-state + the owner's next-choice menu.)** Pushed develop→staging: both services `500eaf9`, `/health/ready` db:up, BOTH migrations applied to `gifsy_staging` (foundation + credit-code-per-tenant; guarded read confirmed 3 tables + 4 CreditPayoutEntry cols + 3 unique idx). API-surface verified LIVE (config PUT/GET round-trip persists distinct DEDUCT/GROSS_UP; reports+gst-reimbursements 200 w/ {paise,inr}; RBAC negatives 403; Deoleo login 200 byte-identical). Deoleo staging tdsPolicy=194C GROSS_UP (armed for UAT; no-op until a VISIBILITY payout flows). **REMAINING = owner UAT** (per-role click-through + real payout-Excel→confirm→invoice→ledger→UTR-lock write-path E2E) **→ W4 prod cutover.** Full build: W0 schema/migration + W0.5 contracts + W1 backend (dual+dual+final money-audit CLEAN) + W2 FE (5 admin pages under "TDS & Invoicing" nav + admin/invoices tenant-read-only + partner-copy D13/D14 fix). Gate: api nest 0/jest 2023 · FE tsc 0/vitest 2039. **DEFERRED DECISIONS (owner-parked) in WAVE0-SCHEMA §10:** DD-1 (owner "keep as-is" 2026-07-29) tenant recovery report exposes cross-tenant PAN aggregate (`panBase`/`panTdsTotal`) → a tenant can infer amounts a SHARED-PAN retailer got from OTHER 194C tenants (amounts only, no identity; dormant till 2nd 194C tenant); **✅ DD-2 double-reversal + DD-3 no-PAN-per-(client,outlet) + MIXED cross-tenant methodology EXERCISED + CLOSED 2026-07-29** (synthetic 2-tenant staging run, real API full write-path, then fully deleted; numbers paise-exact); **NEW ⚠️ DD-4 (CA-review): gross-up TDS invoice carries GST for GST-registered retailers**.
  ...
  advisory-lock code-gen. **✅ Double-reversal edge (DD-2) + no-PAN-per-(client,outlet)/MIXED (DD-3) now EXERCISED + CLOSED (2026-07-29 synthetic 2-tenant staging run, paise-exact, then deleted); NEW ⚠️ DD-4 gross-up-TDS-invoice-carries-GST → CA-review.** *(historical GAPS now built:)* (1) per-tenant **config** `{visibilityPayoutSection 194R|194C,
...
> **⏳ PENDING #17 (owner-gated, NOT done):** develop `c2f851a` is **7 commits ahead** of prod `4ebf12c` — the next cutover ships, together: POSM post-cutover infra (`0615af2`), the VISIBILITY-LED-PAYOUTS/194C-TDS feature (`500eaf9`, **+2 additive migrations** `20260728120000_visibility_payout_tds_foundation` + `20260728130000_credit_code_per_tenant_unique`), and the GIFSY assume-tenant scoping fix (`706efd1`+`12045db`). Both features already LIVE + verified on staging. Gate at HEAD: api nest 0 / jest 2056 · FE tsc 0 / vitest 2039. Awaiting owner UAT → merge→main → owner-approved gate.
...
**🟢 TWO deliverables sit on `develop`/STAGING, NOTHING on prod — both ride the NEXT owner-gated prod cutover. develop = origin/develop = `c2f851a` (7 commits ahead of prod `main` = `4ebf12c`).**
...
- **(c) Owner-gated prod cutover** — FF merge develop→main; carries the **7-commit bundle**: POSM post-cutover infra (`0615af2` — VISIBILITY_REMINDER_SECRET + scheduler + GCS lifecycle now reach the prod deploy workflows), the TDS feature + **2 ADDITIVE migrations**, the assume-tenant scoping fix. Prod pre-check: migrations are additive-safe; post-cutover verify serving SHA + `/health/ready` + a TDS config round-trip + the assumed-scope behaviour.

**PARKED / CA-review (NOT blockers):** DD-1 tenant recovery report exposes the cross-tenant PAN aggregate (`panBase`/`panTdsTotal`) to a tenant admin — **owner KEEP-AS-IS** (fix = strip those 2 fields when scope.clientId set). **✅ DD-2 (double-reversal) + DD-3 (no-PAN per-(clientId,outletCode) + MIXED cross-tenant methodology) EXERCISED + CLOSED 2026-07-29** via a synthetic 2-tenant staging run (2 tenants GROSS_UP+DEDUCT, shared PAN + no-PAN outlets, full write-path via real API, guarded-read verified, then fully deleted — 0 rows remain; numbers matched hand-calc to the paise: MIXED shared-PAN liability ₹904 = ₹500 deduct + ₹404 gross-up; no-PAN A2 ₹40k recovered ₹10k, B2 ₹20k below-per-outlet-threshold NOT; DD-2 one −₹400 reversal, re-apply added no 2nd row). **NEW ⚠️ DD-4 (CA-review, from the run):** the gross-up **TDS invoice** for a GST-REGISTERED retailer is created WITH GST (₹72.72 held on a ₹404 TDS invoice; unregistered correctly gst=0) — genuine tax-treatment question, not a Deoleo blocker; fix-if-needed = skip computeGST/GstReimbursement for `invoiceKind=TDS`. All in WAVE0-SCHEMA §10. (batchCode/downloadCode global-unique collision was FIXED this wave. DB-level RLS still backlog Gap #23 — isolation is application-`clientId`-scoped.)
