...
## 🟢 CURRENT STATE
- **🔧 DEVELOP IS AHEAD OF PROD (#20) WITH TWO ADDITIVE + DORMANT SCHEME BATCHES (post-#20 owner UAT).** Verify HEADs via `git log`. **local develop = origin/develop = `369df10`** (Batch 2 is now PUSHED + on staging — no longer local-only) · **prod = main = `8c08af3`** (#20). Both batches are additive + dormant (Deoleo live path byte-identical until a Gifsy admin uses a scheme with these features), both now LIVE ON STAGING, both ride the owner-gated cutover #21.
  - **BATCH 1 — "UAT fixes" (`86c1a99`,`2ae2c6b`,`21bcfdc`,`c510cc0`) — LIVE ON STAGING, gate-green, adversarially AUDITED CLEAN, CODE-ONLY (no migration), NOT yet in prod.** Contents: (1) **Reach fix** — `assertSalesReachRoster` (scheme-enrollment.service) now authorizes a SALES enroll/OTP if the tagged employee OR the matched outlet/partner is assigned to ANYONE in the caller's whole **DOWNLINE** (was direct-assignment only) → fixes the false "This outlet is not within your team" on an outlet that legitimately shows in the rep's task window; audited CLEAN (tenant-safe, standalone still blocked, consistent with getSalesTargets). (2) **Camera-only live capture** — the CAMERA field is now an in-app getUserMedia camera (webcam/rear-cam), capture/retake, NO gallery/file fallback, clear denied-permission error; DOCUMENT/IMAGE/UPI_QR keep the file picker. (3) **Roster Outlet ID/Name in prefill** — `withRosterIdentity` exposes the roster row's own Outlet ID + Name so a DATA_DISPLAY bound to them resolves (matched OR standalone); getPrefillSources lists them first. (4) **Persistent roster view + Download roster (.xlsx)** — "Current roster" table + Download button under Audience (`GET :id/roster/export`, GIFSY-only, tenant-scoped); getRoster now joins the tagged employee code. (5) **Data-Display "Excel column" is now a DROPDOWN** of uploaded roster columns (was free text). (6) roster-export base-column/prefill-name collision fix (audit LOW). Gate: api jest 2107 / nest 0 / FE tsc 0 / vitest 2049. **REMAINING for Batch 1 = owner staging re-test** (esp. Send-OTP on the previously-blocked outlet + camera on a REAL device) + the ~5–10 min camera/geo/signature phone smoke → then it rides cutover #21 (or a separate code-only cutover).
  - **BATCH 2 — "C: edit/delete a filled enrollment" (`369df10`) — ✅ BUILT + DUAL-AUDITED + FIXED + RE-AUDITED CLEAN + GATE-GREEN + PUSHED TO STAGING + MIGRATION APPLIED TO STAGING + STAGING-RUNTIME-VERIFIED. NOT yet in prod (awaits owner-gated cutover #21). CARRIES A MIGRATION.** Owner decisions (locked): admin edit → saves as a NEW version (append, trail kept); enroller edit → allowed ONLY when the admin turns on a per-scheme flag `audienceConfig.allowEnrollerEdit`; delete → soft-delete that FREES the roster row to re-enroll (recoverable); **SALES-rep self-edit DISABLED**. Design (avoids a risky change to the enrollment 1:1-unique): additive `SchemeEnrollment.deletedAt` column + "reset-on-reenroll" — delete sets `deletedAt` (hidden everywhere); re-enrolling that outlet RESETS the same row (clears `deletedAt` + appends a version); history retained; recoverable. **Migration `20260730160000_scheme_enrollment_soft_delete`** (additive nullable `deletedAt` + `scheme_enrollments_deletedAt_idx`) — **✅ APPLIED to `gifsy_staging`** (column present nullable + index present + `_prisma_migrations` finished_at set / rolled_back_at null → Prisma-recorded, no drift; applied by the staging pipeline's `gifsy-migrate-staging` in-VPC job, which runs `prisma migrate deploy` BEFORE the api deploy → no broken-read window). **NOT applied to prod.** Backend (scheme-enrollment.service + controller + scheme-admin.service + scheme-report.service + dto): admin edit `PUT :id/enrollments/:enrollmentId` (→ new version) · `DELETE :id/enrollments/:enrollmentId` (soft-delete) + `POST :id/enrollments/:enrollmentId/restore` · GIFSY-only `GET :id/enrollments/deleted` (`adminListDeletedEnrollments`, never folded into the normal list) · `enroll()` re-enrolls a soft-deleted row (resets it) · `resubmit()` allows a SUBMITTED enrollment when `allowEnrollerEdit` · hide-soft-deleted in EVERY read/export · `SetAudienceDto.allowEnrollerEdit`. FE: admin drawer **Edit / Delete / Undo** + a persistent **"Show deleted"** panel with per-row **Restore** · **"Allow enroller to edit their own submission"** audience toggle · **partner** self-edit WORKS (resubmit, pre-fills prior values) · **SALES-rep self-edit DISABLED**.
    - **DUAL AUDIT + FIXES (all in `369df10`; independent third-pass re-audit came back CLEAN):** **HIGH (both auditors)** — both edit paths (admin `adminEditEnrollment` + enroller `resubmit` of a SUBMITTED row) routed through `submit()`'s PHONE_OTP consent gate demanding a fresh OTP the edit forms can't supply → edit was NON-FUNCTIONAL (403/400) on any consent-gated scheme; FIX = `submit()` gains `consentedEditFrom` and on an edit REUSES the phone OTP-verified at the ORIGINAL capture (immutable; client-sent phone discarded), never a fresh OTP; fail-closed (no valid prior phone → falls through to the real OTP gate); fresh enroll / rejection-resubmit / re-enroll-of-soft-deleted pass null → fresh consent still required (byte-identical). **MED** — "SALES self-edit disabled" was FE-only; backend `resubmit` SUBMITTED-branch now server-gates on `enrollmentMode === 'SELF'` before authorizeRoster (REJECTED resubmit any mode untouched). **MED** — soft-delete was only recoverable inside the 8s Undo toast; FIX = the dedicated GIFSY-only `GET :id/enrollments/deleted` + persistent "Show deleted" restore panel. **LOW** — admin edit form now treats PHONE_OTP + any object-valued field (media/GPS) as uneditable. **Leak hunt:** both auditors enumerated all 15 enrollment read/report/export/coverage sites — every one filters `deletedAt`. Clean. **3 unit tests added** (SELF-mode gate blocks SALES-mode SUBMITTED edit · allowEnrollerEdit SELF happy path bumps version · admin-edit consent carry-forward reuses the prior verified phone, ignores a client-substituted number). **Gate green: api jest 2110 · nest 0 · FE tsc 0 · vitest 2049.**
    - **STAGING RUNTIME-VERIFIED (real GIFSY admin OTP login → assume deoleo → real HTTP against a synthetic PHONE_OTP scheme → DB-verified → cleaned up):** admin edit of a PHONE_OTP enrollment SUCCEEDS with NO fresh OTP (the HIGH fix — pre-fix it 403'd) → new version v2 · a client-substituted phone (9999999999) was DISCARDED, DB stored the originally-verified 9812300011 (consent carried forward, immutable; history v1+v2 consistent) · DELETE hides the enrollment from the admin list (→null) AND the report (enrolled 1→0, submitted 1→0) · `GET :id/enrollments/deleted` lists the soft-deleted v2 · RESTORE brings it back (SUBMITTED v2) · all new routes (PUT/DELETE/restore/deleted) 401-wired (not 404). **NOT live-exercised but covered by the 3 unit tests + dual audit + clean re-audit** (needs an enroller identity tagged to the outlet): the SELF-mode gate blocking a SALES-mode enroller-edit, a partner `allowEnrollerEdit` self-edit, and the reset-same-row-on-reenroll (delete-frees-the-row IS proven live; the reset is audit+unit-covered). **REMAINING for Batch 2 = the owner-gated cutover #21** (carries the migration; steps 1–5 of the old NEXT-ACTION list are DONE).
- **✅ CUTOVER #20 LIVE (`8c08af3`, 2026-07-30) — Scheme UX-hardening + Parent-ID export; prod=main=`8c08af3` (develop is now AHEAD — see the batches bullet above); code-only/no-migration; additive+dormant.** Both services (gifsy-api, gifsy-frontend) serve `8c08af3`; `/health/ready` 200 `{db:up}`; new reads `/v1/schemes/:id/prefill-sources` + `/facet-values` + `/broadcast/preview` and the outlet-master export (now with the Parent ID column) all 401-wired; `deoleoloyalty.gifsy.in/auth/login` 200. Payload = **SCHEME UX-HARDENING** (dual-source prefill `FormField.outletField` + H1–H6 + must-fixes) + the **PARENT ID outlet-master DOWNLOAD** export fix (`reports.service.outletMaster`, 57 cols, round-trips upload). Dual-audited (BE + FE) + staging-verified 21/21. **Additive + DORMANT** — Deoleo live path byte-identical until a Gifsy admin uses the new scheme features. REMAINING = owner ~5–10 min real-phone smoke (signature canvas, camera/geo, dual-source locked prefill on a live enroll). Rollback ref `daa4f3f` (#19). Verify all HEADs via `git log`. *(See ledger row #20.)*
...
- **prod SERVES `8c08af3` (cutover #20, 2026-07-30); main == `8c08af3`. ⚠️ develop is AHEAD** (local develop == origin/develop == `369df10` = Batch 1 + Batch 2, both LIVE ON STAGING; Batch 2's `deletedAt` migration applied to `gifsy_staging`, NOT prod — see 🟢 CURRENT STATE + ▶▶ START HERE). *(history: `daa4f3f` #19, `f193127` #18, `52fc19f` #17.)* Cutover #20 = **scheme UX-hardening + Parent-ID outlet-master export** (dual-source prefill + H1–H6 + must-fixes; code-only, no migration; additive + dormant; see ledger row #20). Cutover #19 = **scheme roster-upload Excel report** (Download-report button on the roster upload result → Summary + per-row disposition + Duplicates + Unmatched-Employees sheets; code-only, no migration; GIFSY-admin-only; audited GO + staging-verified). Cutover #18 = **scheme prefill Editable/Locked** (backend-enforced Excel-variable prefill; memory [[scheme-data-collection]] §16).
...
> **📌 PENDING CUTOVER #21 (Batch 1 + Batch 2 — CARRIES a migration).** Not yet cut. Both batches now LIVE ON STAGING at develop == origin/develop == `369df10`. **Batch 1** ("UAT fixes" — reach fix, camera-only capture, roster view/download .xlsx, DATA_DISPLAY dropdown) code-only, gate-green + audited CLEAN, awaiting the owner's staging re-test. **Batch 2** ("C: edit/delete a filled enrollment") carries the **additive** migration `20260730160000_scheme_enrollment_soft_delete` (`SchemeEnrollment.deletedAt` + `scheme_enrollments_deletedAt_idx`) — **✅ APPLIED to `gifsy_staging`** (not prod); Batch 2 is BUILT + DUAL-AUDITED CLEAN (HIGH consent-carry-forward + 2 MED + LOW fixed in `369df10`; re-audit clean) + gate-green (api jest 2110 · vitest 2049) + STAGING-RUNTIME-VERIFIED. At #21 the migration → prod pre-check + `migrate deploy` (NOT code-only). Both additive + DORMANT. Remaining before #21: owner staging re-test / phone smoke → owner-gated cutover. Rollback ref = `8c08af3`. *(See ▶▶ START HERE.)*
>
> **✅ CUTOVER #20 DONE + VERIFIED (2026-07-30) — prod = main = `8c08af3`.** SCHEME UX-HARDENING (dual-source prefill `FormField.outletField` + H1–H6 + must-fixes: DATA_DISPLAY dataDisplayKey, activation gate, immutable REJECTED reject-audit trail, approved-owner phone pre-pin, GPS-accuracy cap, dropped per-field audience, prefill-sources/facet-values endpoints, report/notify scoping-unify, export TOGGLE→Yes/No + media URL, broadcast preview) + the **PARENT ID outlet-master DOWNLOAD** export fix (`reports.service.outletMaster`, 57 cols, round-trips upload). **CODE-ONLY — no migration.** FF merge develop→main (5 commits), owner-approved gate. Dual-audited (BE + FE) + staging-verified 21/21. Prod-verified: both services `8c08af3`, `/health/ready` db:up, new reads `/v1/schemes/:id/prefill-sources` + `/facet-values` + `/broadcast/preview` + outlet-master export (with Parent ID) all 401-wired, `deoleoloyalty.gifsy.in/auth/login` 200. **Additive + DORMANT** — Deoleo live path byte-identical until a Gifsy admin uses the new scheme features. REMAINING = owner ~5–10 min real-phone smoke. Rollback ref `daa4f3f` (#19). *(See ledger row #20.)*
>
...
**✅ CUTOVER #20 LIVE + VERIFIED (2026-07-30) — prod = main = `8c08af3`** (always verify via `git log`, don't trust a hardcoded SHA). Shipped = **SCHEME UX-HARDENING** (dual-source prefill + H1–H6; dual-audited BE+FE + staging-verified 21/21; code-only, no migration) + the **PARENT ID outlet-master DOWNLOAD** export fix. **⚠️ DEVELOP IS AHEAD OF PROD** with TWO post-#20 owner-UAT batches (both additive + DORMANT, both LIVE ON STAGING at develop == origin/develop == `369df10`): **Batch 1** (UAT fixes — reach fix, camera-only capture, roster view/download, DATA_DISPLAY dropdown; gate-green + audited CLEAN, code-only) awaiting the owner's staging re-test; **Batch 2** (C: edit/delete a filled enrollment; **carries a migration** — `deletedAt`, applied to `gifsy_staging` not prod) is now **BUILT + DUAL-AUDITED CLEAN + GATE-GREEN + STAGING-RUNTIME-VERIFIED**. **NEXT MOVE = the owner-gated cutover #21 (both batches; carries the migration).** Recent cutovers: **#20** scheme UX-hardening + Parent-ID export · **#19** scheme roster-upload Excel report · **#18** scheme prefill Editable/Locked · **#17** TDS/194C + assume-tenant scoping + partner-grouping FE. All additive + DORMANT.

## ▶▶ START HERE (post-compact) — BATCH 2 is DONE through staging-verify; NEXT = owner-gated cutover #21 (carries a migration)
**READ FIRST: memory [[scheme-data-collection]] + `platform/docs/plans/SCHEME-DATA-COLLECTION-DESIGN.md`.** This session (post-#20) surfaced two owner-UAT batches; **both are now LIVE ON STAGING** at develop == origin/develop == `369df10` — Batch 1 awaiting the owner's re-test, Batch 2 built + dual-audited CLEAN + staging-runtime-verified. Full batch contents are in **🟢 CURRENT STATE** at the top of this file. Verify HEADs via `git log` (local develop == origin/develop `369df10` · prod `8c08af3`).

### The active build = BATCH 2 — "C: edit/delete a filled enrollment" (`369df10`, ON STAGING, carries the `deletedAt` migration) — ✅ steps 1–5 DONE
**Owner decisions (locked):** admin edit → NEW version (append, trail kept) · enroller edit → allowed ONLY when the admin turns on per-scheme `audienceConfig.allowEnrollerEdit` · delete → soft-delete that FREES the roster row to re-enroll (recoverable). **Design:** additive `SchemeEnrollment.deletedAt` + "reset-on-reenroll" (delete sets `deletedAt`/hidden everywhere; re-enroll RESETS the same row + appends a version; history retained, recoverable) — avoids a risky change to the enrollment 1:1-unique. **SALES-rep self-edit is DISABLED** (a rep edit would risk a blank overwrite; a GIFSY admin CAN edit sales-filled enrollments; the disable is now server-enforced too — `resubmit` SUBMITTED-branch gates on `enrollmentMode === 'SELF'`). Migration `20260730160000_scheme_enrollment_soft_delete` (additive nullable `deletedAt` + `scheme_enrollments_deletedAt_idx`) **✅ APPLIED to `gifsy_staging`** (column + index present, `_prisma_migrations` finished_at set / rolled_back_at null → no drift; run by the staging pipeline's in-VPC `gifsy-migrate-staging` job BEFORE the api deploy). **NOT applied to prod.**
- **✅ DUAL AUDIT + FIXES (all in `369df10`; independent third-pass re-audit CLEAN):** HIGH (both auditors) — both edit paths routed through `submit()`'s PHONE_OTP consent gate demanding a fresh OTP the edit forms can't supply → edit was NON-FUNCTIONAL (403/400) on any consent-gated scheme; FIX = `submit()` `consentedEditFrom` REUSES the phone OTP-verified at the ORIGINAL capture (immutable; client phone discarded), fail-closed, fresh enroll/rejection-resubmit/re-enroll byte-identical. MED — server-enforced SELF-mode gate on the enroller SUBMITTED-edit (was FE-only). MED — persistent GIFSY-only `GET :id/enrollments/deleted` + "Show deleted" restore panel (was Undo-toast-only). LOW — admin edit form treats PHONE_OTP + object-valued fields as uneditable. Leak hunt: all 15 read/report/export/coverage sites filter `deletedAt` — clean. 3 unit tests added. **Gate green: api jest 2110 · nest 0 · FE tsc 0 · vitest 2049.**
- **✅ STAGING RUNTIME-VERIFIED** (real GIFSY admin OTP → assume deoleo → real HTTP against a synthetic PHONE_OTP scheme → DB-verified → cleaned up): admin edit succeeds with NO fresh OTP → v2 · substituted phone 9999999999 DISCARDED, DB kept the verified 9812300011 (consent carried forward, immutable) · DELETE hides from admin list (→null) AND report (1→0/1→0) · `GET :id/enrollments/deleted` lists it · RESTORE brings back SUBMITTED v2 · all new routes 401-wired. NOT live-exercised but unit+audit-covered: the SELF-mode gate, a partner `allowEnrollerEdit` self-edit, the reset-same-row-on-reenroll (delete-frees-the-row proven live).
- **▶ NEXT ACTION = the OWNER-GATED cutover #21** (see 📌 below). Steps 1–5 of the old build list (FE vitest → dual-audit → push → apply staging migration → staging-verify) are DONE.

### Also on the bench = BATCH 1 — "UAT fixes" (now in `369df10`) — LIVE ON STAGING, awaiting the owner's re-test
Code-only, gate-green (api jest 2107 / nest 0 / FE tsc 0 / vitest 2049), adversarially AUDITED CLEAN, NOT yet in prod. Five fixes (contents in 🟢 CURRENT STATE): **reach fix** (whole-downline SALES enroll/OTP) · **camera-only live capture** (in-app getUserMedia, no gallery fallback) · **roster Outlet ID/Name in prefill** (DATA_DISPLAY resolves) · **persistent roster view + Download roster .xlsx** · **Data-Display "Excel column" → dropdown**. **REMAINING = owner staging re-test** (esp. Send-OTP on the previously-blocked outlet + camera on a REAL device) + the ~5–10 min camera/geo/signature phone smoke → then it rides cutover #21.

### 📌 PENDING CUTOVER #21 (Batch 1 + Batch 2 — CARRIES a migration)
Both batches are now on staging (develop `369df10`) with their gates green and Batch 2 dual-audited CLEAN + staging-runtime-verified; the remaining gate is the owner's staging re-test/phone smoke. Cutover #21 ships Batch 1 (code-only) + Batch 2 (**carries the `20260730160000_scheme_enrollment_soft_delete` migration → prod pre-check + `migrate deploy`, NOT code-only like #20**). Rollback ref = `8c08af3`. Guardrails unchanged: never merge to main / apply a prod-or-staging DB op without the owner; money/auth/OTP write-paths → mandatory dual audit.
