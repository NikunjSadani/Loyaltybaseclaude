...
## 🟢 CURRENT STATE
> ✅ **prod = main = develop = `078c404` (all in sync; develop is NOT ahead of prod anymore). Cutovers #23 (`ce5267a`) + #24 (`078c404`) are LIVE + VERIFIED (2026-08-03) — the WHOLE UI/UX audit fix plan (52 findings, P1–P4+G1) + the prior cutover-#23 stack (roster-remove, Auth Option B, KYC grouped-child phone fix, per-photo geotag) are now LIVE in prod. Nothing pending on develop. Remaining = owner-side on-device smoke only.** The cutover ledger below is the PROD history.
- **✅ CUTOVER #24 LIVE + VERIFIED (`078c404`, 2026-08-03) — prod = main = develop = `078c404`.** FE-only, **NO migration**: scheme "Enroll"→"Select" button rename (the Tasks scheme-card button + the outlet-picker row button — they open selection, not enroll; the real enroll is at Submit) + its spec fix (`scheme-enrollment-flow.test.tsx`). Verified: both services `078c404`, `/health/ready` db:up, login 200. Left AS-IS per owner: the form-view title "Enroll <outlet>" + the success-screen "Enroll another outlet". Rollback ref `ce5267a` (#23).
- **✅ CUTOVER #23 LIVE + VERIFIED (`ce5267a`, 2026-08-03) — the WHOLE UI/UX audit fix plan (52 findings) + the prior #23 stack.** All additive (FE + one migration). Payload: (a) **Scheme roster-row soft-delete** (`378f795`) — **carries migration `20260801120000_scheme_outlet_soft_delete`** (additive `scheme_outlets.deletedAt` nullable col + `scheme_outlets_schemeId_deletedAt_idx` index), **APPLIED + VERIFIED on prod** (guarded `gifsy_prod` read: `done:true, rolled:false`, column present, index present) · (b) **Auth Option B check-first login OTP** (`6ba9ead`, no migration) · (c) **KYC grouped-child shared-phone fix** (`e0f767b`) · (d) **per-photo geotag** (`ee00dd2`+`21fe2c7`; GPS shows as a "(Geo)" column in the enrollment Excel report) · (e) **the WHOLE UI/UX audit fix plan (52 findings): P1 roster-remove `3661a55` · P2 prod-live HIGH `0e14bdb` · P3 prod-live MED `6491f0f` · P4+G1 LOW/a11y+geotag `c9d0f1e`** — every phase DUAL-audited (correctness + UI/UX) · (f) **corrected camera-field helper copy** (`ce5267a`) — removed a false "server-side watermark" claim (no watermark is implemented; the geo is stored + shown in the report, NOT burned into the image). **The dual audit caught 4 HIGH defects a green gate would have hidden, all fixed:** (1) ENR-H4 raw-fetch dropped the `{data:…}` envelope → every form-bearing scheme rendered formless; (2) ENR-M7 checkbox editor dead behind a pre-existing `isObjectVal` guard; (3) KYC-M5 silent post-Back payout-edit loss; (4) SegmentedControl arrow-key auto-committed money-path toggles. **Owner decisions:** D1 = KYC geo stays a HARD block + "Retry location" (not flag-and-allow); H1 = after submit ("Send OTP to Outlet Owner") the KYC form LOCKS + the button relabels "Continue to verification →", corrections go via admin re-KYC (the OTP step only confirms owner consent, carries no form data). Gate at cutover: FE tsc 0 · vitest 2066 · api jest 2195 · nest 0. Prod-verified: both services `ce5267a`, `/health/ready` db:up, migration applied, `/v1/schemes/media/view` bad-token 404 (fail-closed), `park` 401-wired, `deoleoloyalty.gifsy.in/auth/login` 200 + send-otp 200. Rollback ref `fb996d8` (#22).
- **(historical) CUTOVER #22 (`fb996d8`, 2026-07-31) — superseded in prod by #23/#24 above.** Verify HEADs via `git log`. **Gate at cutover: api jest 2163 · nest build 0 · FE vitest 2066 · tsc 0.** Migration `20260731170000_outlet_kyc_intent_parked` (`ALTER TYPE "OutletKycIntent" ADD VALUE 'PARKED'`, additive) **APPLIED to BOTH gifsy_staging and gifsy_prod** (guarded reads confirmed: enum `["NOT_INTERESTED","PARKED"]`, migration recorded/not-rolled-back on prod). Prod-verified: both services `fb996d8`, `/health/ready` db:up, `/v1/admin/outlets/park` + `/unpark` 401-wired, public `/v1/schemes/media/view` bad-token 404 (fail-closed), `deoleoloyalty.gifsy.in/auth/login` 200 + send-otp 200. Rollback ref `a83b2f4` (#21). **All 5 parts additive + DORMANT** — the Deoleo live path is unaffected until an admin parks an outlet or a grouped child inherits a doc. The 5-part payload:
...
> **✅ CUTOVER #24 DONE + VERIFIED (2026-08-03) — prod = main = origin/develop = `078c404` (develop NOT ahead).** FE-only, **NO migration**: scheme "Enroll"→"Select" button rename (Tasks scheme-card button + outlet-picker row button — they open selection, not enroll; the real enroll is at Submit) + its spec fix (`scheme-enrollment-flow.test.tsx`). Verified: both services `078c404`, `/health/ready` db:up, login 200. Left AS-IS per owner: form-view title "Enroll <outlet>" + success-screen "Enroll another outlet". Rollback ref `ce5267a` (#23). *(See ledger row #24.)*
>
> **✅ CUTOVER #23 DONE + VERIFIED (2026-08-03) — `ce5267a` (superseded in prod by #24).** The WHOLE UI/UX audit fix plan (52 findings) + the prior #23 stack; all additive (FE + one migration). Payload: **(a)** scheme roster-row soft-delete (`378f795`) — CARRIES migration `20260801120000_scheme_outlet_soft_delete` (additive `scheme_outlets.deletedAt` col + `scheme_outlets_schemeId_deletedAt_idx`), **APPLIED + VERIFIED on prod** (guarded read `done:true, rolled:false`, col + idx present) · **(b)** Auth Option B check-first login OTP (`6ba9ead`, no migration) · **(c)** KYC grouped-child shared-phone fix (`e0f767b`) · **(d)** per-photo geotag (`ee00dd2`+`21fe2c7`; GPS → "(Geo)" column in the enrollment Excel report) · **(e)** the UI/UX fix plan: P1 roster-remove `3661a55` · P2 prod-live HIGH `0e14bdb` · P3 prod-live MED `6491f0f` · P4+G1 LOW/a11y+geotag `c9d0f1e` — every phase DUAL-audited (correctness + UI/UX) · **(f)** corrected camera-field helper copy (`ce5267a`) — removed a false "server-side watermark" claim (no watermark implemented; geo is stored + shown in the report, NOT burned into the image). **Dual audit caught 4 HIGH a green gate would have hidden, all fixed:** (1) ENR-H4 raw-fetch dropped the `{data:…}` envelope → form-bearing schemes rendered formless; (2) ENR-M7 checkbox editor dead behind `isObjectVal`; (3) KYC-M5 silent post-Back payout-edit loss; (4) SegmentedControl arrow-key auto-committed money-path toggles. **Owner decisions:** D1 KYC geo stays a HARD block + "Retry location" (not flag-and-allow); H1 post-submit KYC form LOCKS + button relabels "Continue to verification →", corrections via admin re-KYC (OTP step only confirms owner consent, carries no form data). **Gate: FE tsc 0 · vitest 2066 · api jest 2195 · nest 0.** Prod-verified: both services `ce5267a`, `/health/ready` db:up, migration applied, `/v1/schemes/media/view` bad-token 404 (fail-closed), `park` 401-wired, `deoleoloyalty.gifsy.in/auth/login` 200 + send-otp 200. **REMAINING (owner-side, nothing blocks Deoleo): on-device smoke — KYC submit-lock behavior + camera/GPS/signature on a real phone.** Rollback ref `fb996d8` (#22). *(See ledger row #23.)*
>
> **✅ CUTOVER #22 DONE + VERIFIED (2026-07-31) — `fb996d8` (superseded in prod by #23/#24).** 5-part payload, ALL additive + DORMANT: (1) **reactivate pending-outlet leak fix** — `admin-outlets.service.reactivate` now requires `deactivatedAt: { not: null }` so a KYC-pending outlet can't be flipped active to bypass KYC · (2) **enrollment Excel report rebuild** — Outlet Name fallback to `prefillValues` aliases + media cells are REAL hyperlinks to a NEW PUBLIC tokenized `GET /v1/schemes/media/view?token=<jwt>` (`typ:'schememedia'` HS256, mirrors KYC docview, cross-tenant-guarded, bare-404) that opens from a downloaded file + columns de-duped by `prefillKey`/`outletField` (supersedes the #21 `e0c254d` note) · (3) **PARKED remove-from-KYC-queue outlet state** — new additive `OutletKycIntent` value `PARKED` (distinct from rep-driven `NOT_INTERESTED`); admin bulk `POST /v1/admin/outlets/park` + `/unpark` (GIFSY/CLIENT_ADMIN + `partners:manage_outlets`, max 500) + "Parked / Removed" admin tab; FULLY hidden from reps (sales buildOutlets/rollups/getMember + scheme reach + admin dashboards + visibility universes); KYC approval UN-PARKS (shared activation `updateMany` clears `kycIntent`); `park` idempotent + scoped to true-pending `isActive:false, deactivatedAt:null` · (4) **grouped-child GST-cert/cheque doc carry-forward** — a FIRST-KYC grouped child keeping the group's UNCHANGED GST/bank inherits the APPROVED source's cert/cheque (source = `resolveGroupIdentity.sourcePartnerId` + new `resolveGroupCarryForwardDocs`); backend-authoritative pre-tx resolve+validate+attach-from-stash + a divergence-reject safety net; PAN-present required; FE first-KYC-only gate relax + "Inherited from group (approved)" note · (5) **"Activations / Tasks" label rename** (sales dashboard card + Tasks page + enrollment-sheet header). **CARRIES migration `20260731170000_outlet_kyc_intent_parked`** (`ALTER TYPE "OutletKycIntent" ADD VALUE 'PARKED'`, additive) — **APPLIED to BOTH staging + prod** (guarded reads: enum `["NOT_INTERESTED","PARKED"]`, recorded/not-rolled-back). **Gate: api jest 2163 · nest 0 · FE vitest 2066 · tsc 0.** Dual-audited (parts 3+4). Prod-verified: both services `fb996d8`, `/health/ready` db:up, `/v1/admin/outlets/park` + `/unpark` 401-wired, public `/v1/schemes/media/view` bad-token 404 (fail-closed), `deoleoloyalty.gifsy.in/auth/login` 200 + send-otp 200. Rollback ref `a83b2f4` (#21). *(See ledger row #22 + ▶▶ START HERE.)*
>
...
**✅ CUTOVERS #23 (`ce5267a`) + #24 (`078c404`) LIVE + VERIFIED (2026-08-03) — prod = main = origin/develop = `078c404` (develop is NOT ahead of prod)** (always verify via `git log`, don't trust a hardcoded SHA). #23 shipped the WHOLE UI/UX audit fix plan (52 findings, P1–P4+G1) + the prior #23 stack (scheme roster-row soft-delete carrying migration `20260801120000_scheme_outlet_soft_delete`, applied+verified on prod · Auth Option B check-first login OTP · KYC grouped-child shared-phone fix · per-photo geotag) + a camera-field copy correction (removed a false "server-side watermark" claim). #24 shipped the scheme "Enroll"→"Select" button rename (FE-only, no migration). Every UI/UX phase was DUAL-audited (correctness + UI/UX); the audit caught 4 HIGH defects a green gate would have hidden, all fixed (ENR-H4 envelope drop → formless form-bearing schemes · ENR-M7 dead checkbox editor · KYC-M5 silent post-Back payout-edit loss · SegmentedControl arrow-key auto-commit). Owner decisions: D1 = KYC geo stays a HARD block + "Retry location"; H1 = post-submit KYC form LOCKS + button "Continue to verification →", corrections via admin re-KYC. Gate at #23: FE tsc 0 · vitest 2066 · api jest 2195 · nest 0. **There is NO pending build — develop == main == `078c404`.** Recent cutovers: **#24** scheme Enroll→Select rename · **#23** whole UI/UX fix plan + roster-remove/AuthB/KYC-phone/geotag + copy fix · **#22** reactivate-leak/report-rebuild/PARKED/doc-carry-forward/rename · **#21** scheme UAT batches + child-KYC prefill + uniqueness toggle · **#20** scheme UX-hardening + Parent-ID export · **#19** scheme roster-upload Excel report. **OPEN owner items (their choice among the leftovers): on-device smoke (KYC submit-lock behavior + camera/GPS/signature on a real phone) · UAT of the older dormant surfaces (TDS payout write-path, grouping UI, visibility POSM, scheme).** *(Bank/UPI prod uniqueness flip ✅ DONE 2026-07-31 — deoleo all-on, verified.)*

## ▶▶ START HERE (2026-08-03) — the UI/UX AUDIT FIX PLAN is ✅ LIVE IN PROD (cutover #23); NEXT = owner on-device smoke only
**prod = main = develop = `078c404` (all in sync). Cutovers #23 (`ce5267a`) + #24 (`078c404`) LIVE + VERIFIED (2026-08-03) — the WHOLE UI/UX fix plan + the prior cutover-#23 stack are now IN PROD; develop == main (nothing pending).** Verify HEADs via `git log`. Gate at #23: **api jest 2195 · nest 0 · FE tsc 0 · vitest 2066.**

### ▶▶ UI/UX AUDIT FIX PLAN — ✅ ALL 52 FINDINGS BUILT + DUAL-AUDITED + LIVE IN PROD (cutover #23 `ce5267a`, 2026-08-03)
Owner-directed ("we will start with this entire thing after compact"). Orchestrated multi-agent build, **dual-audit (correctness + UI/UX) per phase**, full gate each; shipped to prod in cutover #23. Full per-item table + as-built banner = **`platform/docs/plans/UIUX-AUDIT-FIXPLAN.md`** (READ FIRST). Commits (now in prod via #23):
- **P1** roster-remove polish (8) — `3661a55`
- **P2** prod-live HIGH bugs (10: SIG-1/2, SET-H2, ENR-H3/H4, KYC-H1/H2/H4, ENR-M1/KYC-up) — `0e14bdb`
- **P3** prod-live MED (14) + backend `formFields[].options` (ENR-M7, additive) — `6491f0f`
- **P4 (16 LOW/a11y) + G1 (3 geotag)** — `c9d0f1e`
- **FE-only + one additive backend projection field — NO migration.**
**The dual audit caught 4 HIGH a green gate would have hidden, all fixed:** P2 ENR-H4 envelope drop (form-bearing schemes → formless), P3 ENR-M7 editor dead behind `isObjectVal`, P3 KYC-M5 silent post-Back payout-edit loss, SegmentedControl arrow-key auto-committed money-path toggles. See [[audit-every-build-item]].
- **✅ D1 DECIDED (owner, 2026-08-02): KYC geo stays a HARD block + a "Retry location" button** (NOT flag-and-allow). Deliberate divergence from the geotag `GEO_UNVERIFIABLE` direction — KYC keeps strongest at-source geo integrity.
- **⚠️ H1 behavior note (KYC-M5):** after submit ("Send OTP to Outlet Owner") the KYC form is now LOCKED (button → "Continue to verification →"); post-submit corrections go via admin re-KYC (the OTP step only confirms owner consent, carries no form data). If the owner wants post-submit editability instead, that's a separate enhancement (re-submit-as-update; needs backend upsert) — surfaced, not built.
- **▶ REMAINING (owner-side, nothing blocks Deoleo):** on-device smoke only — the KYC submit-lock behavior + camera/GPS/signature (and OTP) on a real phone. Everything else is verified in prod.

### ✅ SHIPPED IN CUTOVER #23 (`ce5267a`, 2026-08-03) — the prior stack the UI/UX fix plan sat on top of, all now LIVE IN PROD:
1. **SCHEME ROSTER-ROW REMOVE** (`378f795`, SchemeOutlet soft-delete; "removed stays removed"; triple-audited). **Carried additive migration `20260801120000_scheme_outlet_soft_delete`** — **APPLIED + VERIFIED on prod** (guarded `gifsy_prod` read `done:true, rolled:false`; `scheme_outlets.deletedAt` col + `scheme_outlets_schemeId_deletedAt_idx` index present). Design `SCHEME-ROSTER-ROW-REMOVE-PLAN.md`.
2. **AUTH OPTION B — check-first login OTP** (`6ba9ead`) — `sendOtp` refuses "No account found" if the tenant-scoped number is unregistered; login-only, no FE/migration; dual-audited.
3. **KYC GROUPED-CHILD SHARED-PHONE FIX** (`e0f767b`) — the sales new-KYC mobile-conflict check is now GROUP-AWARE (a phone shared with a SAME-GROUP sibling is NOT flagged, mirroring backend `assertPhoneAvailable`; `sales.service` returns `Outlet.parentId`). Fixed the owner-reported "subsequent grouped outlet can't continue KYC" — the tester was blocked at the MOBILE field (shared owner phone `7755889900`), NOT the GST doc. FE-only, no migration.
4. **PER-PHOTO GEOTAG** (`ee00dd2` build + `21fe2c7` UX fixes) — each CAMERA photo captures its OWN GPS at shutter; per-photo visibility geo-fence; replaces the never-wired `ON_PHOTO` label; GPS shows as a "(Geo)" column in the enrollment Excel report. NO migration (geo in `formValues` JSON). Backend anti-fraud DUAL-audited (2 HIGH+2 LOW fixed: geoFenceOk=true only genuinely-verified-inside; tokenized report link no longer 404s geotagged photos) + FE UI/UX audited (2 HIGH fixed: visibility double-capture race + silent-no-location hint). New `geo-fence.helper.ts` (18-test spec). **Camera-field helper copy corrected at cutover (`ce5267a`)** — removed a false "server-side watermark" claim (no watermark implemented; the geo is stored + shown in the report, NOT burned into the image). Design `PER-PHOTO-GEOTAG-PLAN.md`; memory [[visibility-posm]]. ⚠️ residual owner smoke: camera/GPS on a real phone.

### ▶ PLANS WRITTEN, NOT built (lower priority than the UI/UX fix plan):
- **RBAC completion** (`RBAC-COMPLETION-PLAN.md`) — enable fine-grained perms; ⚠️ flipping today 403s ALL sales+partner routes (their default perm sets don't cover their routes). D1 (scope)/D2 (customization). ~2.5–3.5d (D1-a+D2-a) +1.5–2d (D2-b).

**Other OPEN items (owner picks; no rush, nothing blocks Deoleo):**
