...
## ▶▶ START HERE (post-compact) — develop is AHEAD of prod by TWO built+verified features; owner picks next
**prod = main = `fb996d8` (cutover #22 LIVE, 2026-07-31). develop = `6ba9ead` — AHEAD of prod by TWO features + docs.** Verify HEADs via `git log`. Both are built + audited + **staging-runtime-verified** + gate-green, stacked on develop, **awaiting owner-gated prod cutover #23**. Gate on develop: **api jest 2177/0 · nest 0 · FE tsc 0 · vitest 2066.**

**▶ CUTOVER #23 (owner-gated, whenever) carries BOTH:**
1. **SCHEME ROSTER-ROW REMOVE (SchemeOutlet soft-delete)** — GIFSY admin Remove roster rows (per-row+bulk, hidden-enrollment warning) + Restore from a "Show removed" panel; removed row vanishes from every read; **"removed stays removed"** (no rep/lazy-enroll or filter-resave resurrect — only admin Restore or Excel re-upload naming it). Triple-audited (1 HIGH+1 MED+3 LOW fixed) + **staging 16/16**. **Carries additive migration `20260801120000_scheme_outlet_soft_delete`** (deletedAt nullable + idx) — **applied to gifsy_staging (guarded); NOT prod (applies at cutover via deploy.yml)**. Design = `SCHEME-ROSTER-ROW-REMOVE-PLAN.md`.
2. **AUTH OPTION B — check-first login OTP** (`6ba9ead`) — `sendOtp` now looks up the tenant-scoped account BEFORE sending and refuses "No account found. Please contact your sales representative..." if the number isn't registered under this tenant (owner decision; clearer UX than a silent undelivered SMS on a mistyped number). Scoped to LOGIN only (`authService.sendOtp` — scheme/redemption/KYC OTPs use `msg91.sendOtp`, untouched); index-backed; runs under FIXED_OTP; skipped only when clientId absent (legacy). **No FE change, NO migration.** Dual-audited (security + correctness) CLEAN + **staging-verified 5/5** (registered→200, unregistered→401 w/ msg, cross-tenant→401, no-clientId→200). Enumeration = owner-accepted trade-off (closed KYC-only platform; per-IP @Throttle 5/60s bounds it in prod). Also on develop: targets date-rot test fix (test-only) + bank/UPI caveat-closure + staging-E2E to-do + RBAC/geotag plan docs.

**▶ TWO PLANS WRITTEN, awaiting owner GO + decisions (NOT built — owner said "will share what to go ahead with"):**
- **Per-photo geotag** (`PER-PHOTO-GEOTAG-PLAN.md`) — owner chose Option A: each CAMERA photo embeds its own GPS at shutter time (replaces the never-wired "Bound to a photo" `GPS_POINT` trigger); shared renderer → schemes + visibility; upgrades the visibility geo-fence to per-photo (anti-fraud → dual audit). Decisions D1 (fence rule w/ 2+ photos: rec=all-inside) · D2 (per-photo accuracy cap: rec=yes) · D3 (default-ON for visibility). **Effort ~2.5–3 days + owner ~10-min real-phone smoke.** ⚠️ FINDING: "Bound to a photo" in the current build is a NOT-WIRED label (renders like Manual; never fires on photo capture; one fix per submission) — see the plan.
- **RBAC completion** (`RBAC-COMPLETION-PLAN.md`) — finish/enable the fine-grained `@RequirePermission` layer. ⚠️ Survey found flipping enforcement TODAY would 403 the ENTIRE sales + partner portal (their routes carry permission keys those roles' default sets lack — sales=visibility-only, partner=empty by design). Decisions D1 (scope: rec=admin-portal granularity, sales/partner made permission-safe) · D2 (customization: rec=corrected-defaults now, override admin-UI later). **Effort ~2.5–3.5 days (D1-a+D2-a); +1.5–2 days for override storage+matrix UI (D2-b).** Additive+dormant while building.
- Everything from cutover #22 (reactivate leak fix, enrollment Excel report rebuild, PARKED state, grouped-child doc carry-forward, "Activations / Tasks" rename) is additive + DORMANT and **live in prod**. Full payload = **🟢 CURRENT STATE** at the top of this file.
