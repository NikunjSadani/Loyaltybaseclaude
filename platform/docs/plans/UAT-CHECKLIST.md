...
| 0.4 | Have the **test phone numbers** for each role ready. **Staging OTP is `123456` for every number (FIXED_OTP, re-enabled 2026-06-22) — no real SIM/SMS needed.** Confirm the phones against the **current staging seed** (these have drifted — verify, don't assume): GIFSY admin `9830011252`/clientId gifsy, Deoleo CLIENT_ADMIN `6289864191`, partner+outlet `7795096288`, (optional) MIS. | Each number logs in with OTP `123456`. | |
...
| 12.4 | 🟠 | [operator] | **Throttler is in-memory per-instance.** Rate-limiting is `@nestjs/throttler` in-memory, so counters do **not** aggregate across Cloud Run instances (Redis was removed 2026-07-22 — never backed the throttler). With prod scaled to >1 instance, confirm this per-instance behaviour is acceptable for launch, or plan a shared-store (DB) backing. | Known limitation accepted/documented (readiness §3.2, `GLm-5`); no Redis dependency. | |
