...
async function buildOutlets(salesUserId: string) {
  const now        = new Date()
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!salesUser) return ok({ outlets: [] })

    return ok({ outlets: await buildOutlets(salesUser.id) })
  } catch (e: any) {
    console.error('[sales/outlets GET]', e)
