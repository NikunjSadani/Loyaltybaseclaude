...
export async function GET(req: NextRequest) {
  try {
    ...
    const denied = await requirePermission(authUser as { role: string; clientId: string },'rewards:read');
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const clientId = getClientIdFromRequest(req);
    const denied = await requirePermission(authUser as { role: string; clientId: string },'rewards:manage_inventory');
    if (denied) return denied;
